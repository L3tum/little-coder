...
Use `/deep-plan` for a subagent-focused planning pipeline: RESEARCH, COMPOSE (draft), dual parallel review (REVIEW-PLAN fact-check + REVIEW-PLAN-PONYTAIL lazy-engineering), and COMPOSE (final). After approval a fresh EXECUTION subagent implements the plan instead of the main agent.
