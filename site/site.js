...
(() => {
  ...
  const benchObserver = new IntersectionObserver(
    (entries) => {
      entries.forEach((e) => {
        if (!e.isIntersecting) return;
        e.target
          .querySelectorAll(".bench-bar")
  ...
  document
    .querySelectorAll('[data-anim="bench"]')
    .forEach((el) => benchObserver.observe(el));

  // community counters (separate, simpler)
  const commObserver = new IntersectionObserver(
    (entries) => {
      entries.forEach((e) => {
        if (!e.isIntersecting) return;
        e.target.querySelectorAll(".count").forEach((c) => {
  ...
  document
    .querySelectorAll(".comm-stats")
    .forEach((el) => commObserver.observe(el));
  ...
  (async function refreshStats() {
    ...
    try {
      const [repo, all, closed] = await Promise.all([
        ...
        j(
          `https://api.github.com/search/issues?q=repo:${REPO}+type:issue&per_page=1`,
        ),
        j(
  ...
  (async function refreshVersion() {
    try {
      const r = await fetch(
        "https://api.github.com/repos/itayinbarr/little-coder/releases/latest",
        {
          headers: { Accept: "application/vnd.github+json" },
        },
      );
  ...
  function fillInstallFinal(root) {
    root.querySelectorAll(".t-line").forEach((l) => {
      l.style.visibility = "visible";
    });
    root.querySelectorAll(".cmd").forEach((c, i) => {
      c.textContent = INSTALL_CMDS[i] ?? "";
    });
  ...
  function resetSession(root) {
    ...
    root.querySelectorAll(".user").forEach((u) => {
      u.textContent = "";
    });
    root.querySelectorAll(".caret").forEach((c) => c.classList.remove("hide"));
  }
  ...
  window.addEventListener("resize", () => {
    ...
    resizeTimer = setTimeout(() => {
      if (installStarted) playInstall(installRoot);
      else primeInstall(installRoot);
      if (sessionStarted) playSession(sessionRoot);
      else primeSession(sessionRoot);
    }, 250);
  });
  ...
  (function heroRotate() {
    ...
    function pinHeight() {
      ...
      for (const t of TAILS) {
        tail.textContent = t;
        max = Math.max(max, lead.getBoundingClientRect().height);
      }
    ...
    pinHeight();
    if (document.fonts && document.fonts.ready)
      document.fonts.ready.then(pinHeight).catch(() => {});
    let pinTimer = null,
