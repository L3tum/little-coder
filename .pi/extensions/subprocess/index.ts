...
export default function subprocessExtension(pi: ExtensionAPI) {
  pi.registerCommand("subprocesses", {
    description:
      "List managed background subprocesses, or stop one with /subprocesses stop <id>",
    handler: async (args, ctx) => {
      ...
      if (parts[0] === "stop") {
        ...
        ctx.ui.notify(
          ok ? `Stopping subprocess ${id}` : `No subprocess with id ${id}`,
          ok ? "info" : "error",
        );
        return;
      }
      ...
      ctx.ui.notify(
        [
          ...
          ...entries.map(
            (p) =>
              `  ${p.id}  ${p.name}  pid=${p.pid ?? "?"}  ${p.status}  ${formatAge(now - p.startedAt)}  ${p.command} ${p.args.join(" ")}`,
          ),
          "",
