...
let activeWork:
  | {
      ...
      doneText?: string;
      askRequest?: IssueAgentAskRequest;
      kind?: WorkKind;
...
function formatStatus(): string {
  const age = lastStatusAt
    ? ` (${Math.floor((Date.now() - lastStatusAt) / 1000)}s ago)`
    : "";
  const work = activeWork
    ? ` | #${activeWork.issue.number} ${activeWork.issue.title} | ${activeWork.dir}`
    : "";
...
function parseArgs(input = ""): Record<string, string> {
  ...
  const parts =
    input.match(/(?:[^\s"]+|"[^"]*")+/g)?.map((p) => p.replace(/^"|"$/g, "")) ??
    [];
...
function labelValue(labels: string[], key: string): string | undefined {
  ...
  const bracket = labels.find(
    (l) => l.startsWith(`ai[${key}:`) && l.endsWith("]"),
  );
...
async function postIssueComment(
  ...
  const url =
    r.kind === "github"
      ? `${issue.apiUrl}/comments`
      : `${r.base}/repos/${r.owner}/${r.name}/issues/${issue.number}/comments`;
...
async function submitPullRequestReview(
  ...
  await request(url, token, {
    ...
    await request(url, token, {
      method: "POST",
      body: JSON.stringify({ body, event: "APPROVED" }),
    });
  });
}
...
function labelEndpoint(
  repo: string,
  issue: Issue,
): { url: string; kind: "github" | "forgejo" } {
  const r = apiBase(repo);
  return {
    kind: r.kind,
    url:
      r.kind === "github"
        ? `${issue.apiUrl}/labels`
        : `${r.base}/repos/${r.owner}/${r.name}/issues/${issue.number}/labels`,
  };
}
...
async function listRepoLabels(repo: string, token?: string): Promise<any[]> {
  ...
  for (let page = 1; page <= 100; page++) {
    const rows: any[] = await request(
      `${r.base}/repos/${r.owner}/${r.name}/labels?limit=100&page=${page}`,
      token,
    ).catch(() => []);
    out.push(...rows);
...
async function ensureRepoLabel(
  ...
  if (r.kind !== "forgejo") return;
  if (
    (await listRepoLabels(repo, token)).some(
      (x) => String(x?.name ?? "") === label,
    )
  )
    return;
  await request(`${r.base}/repos/${r.owner}/${r.name}/labels`, token, {
    ...
    body: JSON.stringify({
      name: label,
      color: label.startsWith("ai:state/") ? "#1d76db" : "#5319e7",
      description: "Managed by little-coder issue-agent",
    }),
  }).catch(async (err) => {
    const msg = err instanceof Error ? err.message : String(err);
    if (
      (/^409\b/.test(msg) || /^422\b/.test(msg)) &&
      (await listRepoLabels(repo, token)).some(
        (x) => String(x?.name ?? "") === label,
      )
    )
...
async function ensureStandardLabels(cfg: Config): Promise<void> {
  ...
  const labels = [
    ...
    ...["off", "minimal", "low", "medium", "high", "xhigh", "max"].map(
      (level) => `ai:thinking-level/${level}`,
    ),
    ...[0, 1, 2, 3, 4, 5].map((n) => `ai:priority/${n}`),
    ...(cfg.models.planning
      ? [`ai:planning-model/${cfg.models.planning}`]
      : []),
    ...(cfg.models.execution
      ? [`ai:execution-model/${cfg.models.execution}`]
      : []),
    ...cfg.models.fallback.map(
      (model, index) => `ai:fallback-${index + 1}-model/${model}`,
    ),
...
async function removeLabel(
  ...
  const ep = labelEndpoint(repo, issue);
  await request(`${ep.url}/${encodeURIComponent(label)}`, token, {
    method: "DELETE",
  }).catch(() => undefined);
}
...
async function listComments(
  ...
  const url =
    r.kind === "github"
      ? `${issue.apiUrl}/comments?per_page=100`
      : `${r.base}/repos/${r.owner}/${r.name}/issues/${issue.number}/comments?limit=100`;
...
async function createPullRequest(
  ...
  const pr: any = await request(url, token, {
    method: "POST",
    body: JSON.stringify(body),
  });
  const item = normalizePullRequest(pr);
  if (!pr.issue_url)
    item.apiUrl = `${r.base}/repos/${r.owner}/${r.name}/issues/${item.number}`;
  item.labels = [];
  return item;
}

async function setAiState(
  repo: string,
  issue: Issue,
  state: AiState,
  token?: string,
  ...
  for (const label of issue.labels.filter(
    (l) => /^ai:state[:/]/i.test(l) || /^ai\[state:/i.test(l),
  ))
    await removeLabel(repo, issue, label, token);
...
async function listPullRequests(
  ...
  const prs = rows.map((row) => {
    ...
    if (!row.issue_url)
      pr.apiUrl = `${r.base}/repos/${r.owner}/${r.name}/issues/${pr.number}`;
    return pr;
  });
  for (const pr of prs) {
    if (pr.labels.length) continue;
    const issue: any = await request(
      `${r.base}/repos/${r.owner}/${r.name}/issues/${pr.number}`,
      token,
    ).catch(() => undefined);
    if (issue?.labels)
      pr.labels = issue.labels.map((l: any) =>
        typeof l === "string" ? l : l.name,
      );
...
async function setReviewCycle(
  ...
  for (const label of [
    ...issue.labels.filter(
      (l) =>
        l.startsWith(REVIEW_CYCLE_PREFIX) || l.startsWith("ai:review-cycle:"),
    ),
    ...[1, 2, 3].map((n) => `${REVIEW_CYCLE_PREFIX}${n}`),
  ])
    await removeLabel(repo, issue, label, token);
  if (cycle > 0)
    await addLabels(repo, issue, [`${REVIEW_CYCLE_PREFIX}${cycle}`], token);
}

function hasAgentSource(labels: string[]): boolean {
  return labels.some(
    (l) =>
      l.toUpperCase() === AGENT_SOURCE_LABEL.toUpperCase() ||
      labelValue([l], "source")?.toUpperCase() === "AGENT",
  );
}
...
function parseAskContext(body: string): IssueAgentAskRequest | undefined {
  ...
  const match = body.match(
    new RegExp(
      `${escaped}\\s+([^\\s]+)\\s+${ISSUE_AGENT_ASK_END.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}`,
    ),
  );
...
function latestIssueAgentAnswer(
  comments: any[],
): { answer: string; ask: IssueAgentAskRequest } | undefined {
  const indexed = comments.map((comment, index) => ({ comment, index }));
  for (const { comment, index } of indexed.slice().reverse()) {
    const answer = String(comment.body ?? "")
      .match(/^\s*\/answer\s+([\s\S]+)/im)?.[1]
      ?.trim();
    if (!answer) continue;
    const priorAsk = indexed
      .slice(0, index)
      .reverse()
      .map(({ comment }) => parseAskContext(String(comment.body ?? "")))
      .find(Boolean);
    const answeredLater = indexed
      .slice(index + 1)
      .some(({ comment }) =>
        /## AI Plan|## AI Clarification Needed|Approval detected/i.test(
          String(comment.body ?? ""),
        ),
      );
...
function hasUnansweredReviewCommand(comments: any[]): boolean {
  ...
  const relevant = indexed.filter(
    ({ comment }) =>
      /^\s*\/review\b/im.test(String(comment.body ?? "")) ||
      /## AI Review Summary/i.test(String(comment.body ?? "")),
  );
  const reviewCommands = relevant.filter(({ comment }) =>
    /^\s*\/review\b/im.test(String(comment.body ?? "")),
  );
  if (!reviewCommands.length) return false;
  const summaries = relevant.filter(({ comment }) =>
    /## AI Review Summary/i.test(String(comment.body ?? "")),
  );
  const timestamps = relevant.map(({ comment }) =>
    Date.parse(String(comment.created_at ?? comment.updated_at ?? "")),
  );
  ...
  return (
    Math.max(...reviewCommands.map(order)) >
    Math.max(0, ...summaries.map(order))
  );
}
...
function isAutoresearch(issue: Issue): boolean {
  return issue.labels.some(
    (l) => l === "autoresearch" || l === "ai:autoresearch",
  );
}

function autoresearchConfig(issue: Issue): {
  ...
  const value = (key: string) =>
    issue.labels
      .find((l) => l.startsWith(`autoresearch:${key}=`))
      ?.slice(`autoresearch:${key}=`.length);
  return {
    maxIterations: value("max-iterations"),
    metric: value("metric"),
...
function checkout(repo: string, issue: Issue, workdir: string): string {
  ...
  try {
    execFileSync("git", ["clone", repo, dir], { stdio: "ignore" });
  } catch {}
...
function checkoutPullRequest(
  ...
  execFileSync(
    "git",
    ["fetch", "origin", `+${pr.base}:refs/remotes/origin/${pr.base}`],
    { cwd: dir, stdio: "ignore" },
  );
  const remote =
    pr.headRepo &&
    pr.headRepo.replace(/\.git$/, "") !== repo.replace(/\.git$/, "")
      ? pr.headRepo
      : "origin";
  execFileSync(
    "git",
    ["fetch", remote, `+${pr.head}:refs/remotes/pr-head/${pr.number}`],
    { cwd: dir, stdio: "ignore" },
  );
  execFileSync(
    "git",
    ["checkout", "-B", pr.head, `refs/remotes/pr-head/${pr.number}`],
    { cwd: dir, stdio: "ignore" },
  );
  execFileSync("git", ["clean", "-fdx"], { cwd: dir, stdio: "ignore" });
  return dir;
...
function setWorkspacePermissionsDeny(): void {
  ...
  writeFileSync(
    join(dir, "little-coder-workspace-boundary.json"),
    JSON.stringify({ externalFilePolicy: "deny" }, null, 2) + "\n",
  );
}
...
function extractPlan(text: string): string {
  ...
  return source
    ...
    .replace(/(?:^|\n)\s*(?:---\s*)?(?:SUMMARY|EXECUTION|NOTES)\b[\s\S]*$/i, "")
    .trim();
}
...
function issuePrompt(
  ...
  autoresearch: string,
  answerContext?: { answer: string; ask: IssueAgentAskRequest },
): string {
...
function subAgentCommand(): { command: string; prefix: string[] } {
  if (process.env.ISSUE_AGENT_LITTLE_CODER_BIN)
    return {
      command: process.env.ISSUE_AGENT_LITTLE_CODER_BIN,
      prefix: ["--issue-agent-subagent"],
    };
  ...
  if (existsSync(launcher))
    return {
      ...
      prefix: [launcher, "--issue-agent-subagent"],
    };
  return {
    command: process.execPath,
    prefix: process.argv
      .slice(1)
      .filter((arg) => arg !== "--print" && arg !== "--no-session"),
  };
}
...
function handleSubAgentJsonLine(
  line: string,
  emit: (line: string, type?: "info" | "warning" | "error") => void,
  ...
  if (event.type === "tool_execution_end") {
    emit(
      `sub-agent tool ${event.isError ? "failed" : "finished"}: ${event.toolName}`,
  ...
  if (message?.stopReason === "error" || message?.stopReason === "aborted") {
    const err = String(
      message.errorMessage ?? `sub-agent ${message.stopReason}`,
    );
...
async function runSubAgent(
  ...
  if (process.env.ISSUE_AGENT_LITTLE_CODER_BIN) {
    ...
    const child = startSubprocess(subAgent.command, args, {
      name: "issue-agent sub-agent",
      cwd: dir,
      env: {
        ...process.env,
        ISSUE_AGENT_DONE_FILE: marker,
        PI_OFFLINE: "1",
    ...
    const flushStdout = (text: string) => {
      ...
      for (const line of lines.filter(Boolean))
        output =
          handleSubAgentJsonLine(line, emit, seenAssistantText) ?? output;
    };
    ...
    if (stdoutBuffer.trim())
      output =
        handleSubAgentJsonLine(stdoutBuffer.trim(), emit, seenAssistantText) ??
        output;
    ...
    if (code !== 0) {
      for (const line of stderr.split(/\r?\n/).filter(Boolean))
        emit(`sub-agent stderr: ${line}`, "warning");
      const detail =
        stderr.trim() ||
        output.trim() ||
        `sub-agent process exited with code ${code} (no stderr output — check llama.cpp/llama-swap logs for API errors like 502)`;
  ...
  if (result.exitCode !== 0) {
    ...
    const detail =
      result.stderr.trim() ||
      output.trim() ||
      `sub-agent process exited with code ${result.exitCode} (no stderr output — check llama.cpp/llama-swap logs for API errors like 502)`;
...
function parseVerdict(text: string): ReviewVerdict {
  const raw = text
    .trim()
    .match(/^verdict: (approve|comment|request_changes)\b/i)?.[1]
    ?.toLowerCase();
  return raw === "approve" || raw === "request_changes" ? raw : "comment";
}

async function runRequiredSubAgent(
  work: NonNullable<typeof activeWork>,
  prompt: string,
  model: string | undefined,
  thinkingLevel: ThinkingLevel | undefined,
  ...
  for (let attempt = 1; attempt <= MAX_REQUIRED_TOOL_RETRIES; attempt++) {
    ...
    activeWork = work;
    const effectivePrompt = retryContext
      ? `${prompt}\n\n${retryContext}`
      : prompt;
    ...
    emit(
      `sub-agent finished without required ${required} tool marker (attempt ${attempt}/${MAX_REQUIRED_TOOL_RETRIES})`,
      "warning",
    );
  ...
  if (!work.cfg.dryRun) {
    ...
    await postIssueComment(
      work.repo,
      work.issue,
      `AI lifecycle stopped: sub-agent did not call the required ${required} tool after ${MAX_REQUIRED_TOOL_RETRIES} attempts.`,
      work.cfg.token,
    ).catch(() => undefined);
  }
  throw new Error(
    `missing required ${required} tool marker after ${MAX_REQUIRED_TOOL_RETRIES} attempts`,
  );
}

function config(args: Record<string, string>): Config {
  return {
    repos: (args.repos ?? args.repo ?? process.env.ISSUE_AGENT_REPOS ?? "")
      .split(",")
      .map((s) => s.trim())
      .filter(Boolean),
    models: {
    ...
    thinkingLevels: {
      planning: asThinkingLevel(
        args["planning-thinking-level"] ?? args["thinking-level"],
      ),
      execution: asThinkingLevel(
        args["execution-thinking-level"] ?? args["thinking-level"],
      ),
    },
    workdir:
      args.workdir ??
      process.env.ISSUE_AGENT_WORKDIR ??
      join(tmpdir(), "little-coder-issue-agent"),
    token: args.token ?? process.env.GITHUB_TOKEN ?? process.env.FORGEJO_TOKEN,
    intervalMs: Math.max(
      5000,
      Number(args.interval ?? process.env.ISSUE_AGENT_INTERVAL_MS ?? 30000),
    ),
    dryRun:
      isTruthy(args["dry-run"]) || isTruthy(process.env.ISSUE_AGENT_DRY_RUN),
  };
}
...
function acquireLock(cfg: Config): boolean {
  ...
  writeFileSync(
    file,
    JSON.stringify(
      { pid: process.pid, startedAt: Date.now(), repos: cfg.repos },
      null,
      2,
    ),
  );
...
async function queueIssue(
  ...
  issue: Issue,
  cfg: Config,
  emit?: (msg: string, type?: "info" | "warning" | "error") => void,
): Promise<string> {
  ...
  if (!cfg.dryRun && selectedModel && !findModelByName(ctx, selectedModel)) {
    emit?.(
      `Configured ${state === "PLANNING" ? "planning" : "execution"} model not found: ${selectedModel}; skipping #${issue.number} instead of spawning a sub-agent.`,
      "warning",
    );
    return plannedDir;
  }
  ...
  const thinkingLevelOverride =
    state === "PLANNING"
      ? cfg.thinkingLevels.planning
      : state === "EXECUTING"
        ? cfg.thinkingLevels.execution
        : undefined;
  const thinkingLevel = thinkingLevelOf(issue.labels, thinkingLevelOverride);
  const answerContext =
    state === "PLANNING"
      ? latestIssueAgentAnswer(
          await listComments(repo, issue, cfg.token).catch(() => []),
        )
      : undefined;
  ...
  if (cfg.dryRun) {
    emit?.(
      `DRY RUN: Checkout ${repo} at ${branchName(issue)} into ${dir}`,
      "info",
    );
    if (selectedModel)
      emit?.(
        `DRY RUN: Set ${state === "PLANNING" ? "planning" : "execution"} model to ${selectedModel}`,
        "info",
      );
    if (thinkingLevel)
      emit?.(`DRY RUN: Set thinking level to ${thinkingLevel}`, "info");
    emit?.(
      `DRY RUN: Send issue workflow prompt to model and assume ${state === "PLANNING" ? "a plan document" : "an issueAgentDone tool call"}`,
      "info",
    );
    return dir;
  }
  emit?.(
    `Starting isolated sub-agent in ${dir}; its project AGENTS.md/system prompt will be loaded from that checkout.`,
    "info",
  );
  const finalText = await runRequiredSubAgent(
    activeWork,
    prompt,
    selectedModel,
    thinkingLevel,
  ...
  if (work?.askRequest) {
    ...
    emit?.(
      `Posted clarification question for #${work.issue.number}; waiting for /answer`,
      "info",
    );
    return dir;
  }
...
async function commitPushPr(
  ...
  if (work.cfg.dryRun) {
    notify?.(
      `DRY RUN: Commit issue #${work.issue.number}: ${work.issue.title}`,
      "info",
    );
    notify?.(`DRY RUN: Push branch ${branch}`, "info");
    notify?.(
      `DRY RUN: Open pull request for ${branch}${work.doneText ? ` with body: ${work.doneText}` : ""}`,
      "info",
    );
    return {
      createdPr: true,
      message: `DRY RUN: would push ${branch} and open a PR.`,
    };
  }
  ...
  if (!diff)
    return {
      createdPr: false,
      message: "No file changes were present, so no commit/PR was created.",
    };
  execFileSync(
    "git",
    ["commit", "-m", `AI issue #${work.issue.number}: ${work.issue.title}`],
    { cwd: work.dir, stdio: "ignore" },
  );
  ...
  try {
    await addLabels(
      work.repo,
      pr,
      [AGENT_SOURCE_LABEL, stateLabel("REQUESTING_REVIEW")],
      work.cfg.token,
    );
  } catch (err) {
    await setAiState(work.repo, work.issue, "PR_PENDING", work.cfg.token).catch(
      () => undefined,
    );
    ...
    await postIssueComment(
      work.repo,
      work.issue,
      `Opened PR ${pr.url ?? `#${pr.number}`}, but failed to apply AI review labels to it. Leaving the issue in ${stateLabel("PR_PENDING")} to avoid duplicate execution.\n\n${message}`,
      work.cfg.token,
    ).catch(() => undefined);
    throw err;
  }
  return {
    createdPr: true,
    message: `Pushed ${branch} and opened PR: ${pr.url ?? `#${pr.number}`}`,
  };
}
...
async function finishWork(
  ...
  if (stateOf(work.issue.labels) === "PLANNING") {
    ...
    if (work.cfg.dryRun)
      notify?.(
        `DRY RUN: Post plan document to issue #${work.issue.number}: ${body}`,
        "info",
      );
    else await postIssueComment(work.repo, work.issue, body, work.cfg.token);
    if (work.cfg.dryRun)
      notify?.(
        `DRY RUN: Change issue label to ${stateLabel("WAITING_FOR_FEEDBACK")}`,
        "info",
      );
    else
      await setAiState(
    ...
    if (work.cfg.dryRun)
      notify?.(
        `DRY RUN: Post execution-complete document to issue #${work.issue.number}: ${body}`,
        "info",
      );
    else await postIssueComment(work.repo, work.issue, body, work.cfg.token);
    if (result.createdPr) {
      if (work.cfg.dryRun)
        notify?.(
          `DRY RUN: PR opened with labels ${AGENT_SOURCE_LABEL} and ${stateLabel("REQUESTING_REVIEW")}; change issue label to ${stateLabel("PR_PENDING")}`,
          "info",
        );
      else
        await setAiState(work.repo, work.issue, "PR_PENDING", work.cfg.token);
    } else {
      if (work.cfg.dryRun)
        notify?.(
          `DRY RUN: No PR opened; change issue label to ${stateLabel("WAITING_FOR_REVIEW")}`,
          "info",
        );
      else
        await setAiState(
    ...
    if (work.cfg.dryRun)
      notify?.(
        `DRY RUN: Post stopped-without-issueAgentDone document to issue #${work.issue.number}: ${body}`,
        "info",
      );
    else await postIssueComment(work.repo, work.issue, body, work.cfg.token);
  }
...
async function commitRework(
  work: {
    repo: string;
    issue: PullRequestItem;
    cfg: Config;
    dir: string;
    doneText?: string;
  },
  ...
  execFileSync(
    "git",
    ["commit", "-m", `AI review rework for PR #${work.issue.number}`],
    { cwd: work.dir, stdio: "ignore" },
  );
  const remote =
    work.issue.headRepo &&
    work.issue.headRepo.replace(/\.git$/, "") !==
      work.repo.replace(/\.git$/, "")
      ? work.issue.headRepo
      : "origin";
  execFileSync("git", ["push", remote, `HEAD:${work.issue.head}`], {
    cwd: work.dir,
    stdio: "ignore",
  });
}
...
async function queueReview(
  ...
  const dir = cfg.dryRun
    ? join(cfg.workdir, repoSlug(repo).replace("/", "__"))
    : checkoutPullRequest(repo, pr, cfg.workdir);
  activeWork = {
    repo,
    issue: pr,
    cfg,
  ...
  if (cfg.dryRun) {
    ...
    await runRequiredSubAgent(
      activeWork,
      reviewPrompt(repo, pr, dir),
      undefined,
      undefined,
      "verdict",
      emit ?? (() => undefined),
  ...
  if (!work) return dir;
  const reviewText =
    work.doneText ?? "verdict: comment\n\nNo review text was produced.";
  const verdict = parseVerdict(reviewText);
  const cycle = hasAgentSource(pr.labels)
    ? Math.min(reviewCycle(pr.labels) + 1, 3)
    : reviewCycle(pr.labels);
  if (cfg.dryRun) {
    emit?.(
      `DRY RUN: Submit PR review ${verdict} and summary comment for PR #${pr.number}`,
      "info",
    );
  } else {
    ...
    if (hasAgentSource(pr.labels)) {
      ...
      pr.labels = [
        ...pr.labels.filter(
          (l) =>
            !l.startsWith(REVIEW_CYCLE_PREFIX) &&
            !l.startsWith("ai:review-cycle:"),
        ),
        `${REVIEW_CYCLE_PREFIX}${cycle}`,
      ];
    }
  }
...
async function queueRework(
  ...
  if (cfg.dryRun) {
    ...
    await runRequiredSubAgent(
      activeWork,
      reworkPrompt(repo, pr, dir, reviewText),
      undefined,
      undefined,
      "done",
      emit ?? (() => undefined),
...
function normalizedModelName(modelName: string): string {
  return modelName.includes(":") && !modelName.includes("://")
    ? modelName.replace(/:(?:off|minimal|low|medium|high|xhigh|max)$/i, "")
    : modelName;
}
...
async function switchModelByName(
  pi: ExtensionAPI,
  ctx: any,
  modelName: string,
): Promise<boolean> {
...
async function maintainIssueDependencies(
  repo: string,
  issue: Issue,
  cfg: Config,
  notify?: (msg: string, type?: "info" | "warning" | "error") => void,
  ...
  const now = Date.now();
  if ((dependencyCheckedAt.get(key) ?? 0) + DEPENDENCY_CHECK_COOLDOWN_MS > now)
    return;
  ...
  if (!dep) {
    ...
    const existing = issue.labels.find(
      (l) => l === DEPENDENCY_BLOCK_LABEL || l === "blocked-by-dependency",
    );
    if (!existing) return;
    if (cfg.dryRun)
      notify?.(
        `DRY RUN: Remove issue label ${existing} from #${issue.number}`,
        "info",
      );
  ...
  const closed =
    (await issueState(repo, dep, cfg.token).catch(() => "")) === "closed";
  ...
  if (closed) {
    ...
    if (cfg.dryRun)
      notify?.(
        `DRY RUN: Remove issue label ${existing} from #${issue.number}; dependency #${dep} is closed`,
        "info",
      );
    else await removeLabel(repo, issue, existing, cfg.token);
    return;
  ...
  if (
    issue.labels.includes(DEPENDENCY_BLOCK_LABEL) ||
    issue.labels.includes("blocked-by-dependency")
  )
    return;
  if (cfg.dryRun)
    notify?.(
      `DRY RUN: Change issue label to ${DEPENDENCY_BLOCK_LABEL}; dependency #${dep} is not closed`,
      "info",
    );
  else await addLabels(repo, issue, [DEPENDENCY_BLOCK_LABEL], cfg.token);
}

async function maintainIssueLabels(
  cfg: Config,
  notify?: (msg: string, type?: "info" | "warning" | "error") => void,
): Promise<void> {
  for (const repo of cfg.repos) {
    for (const issue of await listIssues(repo, cfg.token)) {
      await maintainIssueDependencies(repo, issue, cfg, notify).catch((err) =>
        notify?.(
          `issue-agent dependency check failed for #${issue.number}: ${err instanceof Error ? err.message : String(err)}`,
          "warning",
        ),
      );
      const retryLabel = issue.labels.find(
        (l) =>
          l.startsWith("ai:retry-after/") || l.startsWith("ai:retry-after:"),
      );
      const usageLimitLabel = issue.labels.find(
        (l) => l === "ai:blocked/usage-limit" || l === "ai:blocked:usage-limit",
      );
      if (usageLimitLabel && retryLabel) {
        ...
        if (Number.isFinite(when) && when <= Date.now()) {
          if (cfg.dryRun) {
            notify?.(
              `DRY RUN: Remove issue labels ${usageLimitLabel}, ${retryLabel}, and provider-status labels from #${issue.number}`,
              "info",
            );
            notify?.(
              `DRY RUN: Post unblock comment to issue #${issue.number}`,
              "info",
            );
          } else {
            ...
            for (const label of issue.labels.filter(
              (l) =>
                l.startsWith("ai:provider-status/") ||
                l.startsWith("ai:provider-status:"),
            ))
              await removeLabel(repo, issue, label, cfg.token);
            await postIssueComment(
              repo,
              issue,
              "Recorded usage-limit reset time has passed; unblocking this issue for retry.",
              cfg.token,
            ).catch(() => undefined);
          }
        }
      ...
      if (answeredAsk) {
        if (cfg.dryRun) {
          notify?.(
            `DRY RUN: Change issue label to ${stateLabel("PLANNING")} after /answer on #${issue.number}`,
            "info",
          );
        } else {
          ...
          await postIssueComment(
            repo,
            issue,
            `Answer detected (\`/answer\`); switching back to \`${stateLabel("PLANNING")}\`.`,
            cfg.token,
          ).catch(() => undefined);
        }
        continue;
      ...
      if (
        comments.some((c) => /^\s*\/approve\b/im.test(String(c.body ?? "")))
      ) {
        if (cfg.dryRun) {
          notify?.(
            `DRY RUN: Change issue label to ${stateLabel("EXECUTING")}`,
            "info",
          );
          notify?.(
            `DRY RUN: Post approval-detected comment to issue #${issue.number}`,
            "info",
          );
        } else {
          ...
          await postIssueComment(
            repo,
            issue,
            `Approval detected (\`/approve\`); switching to \`${stateLabel("EXECUTING")}\`.`,
            cfg.token,
          ).catch(() => undefined);
        }
      }
...
async function runLoop(
  ...
  if (cfg.dryRun) {
    ...
    if (!acquireLock(cfg)) {
      notify(
        `issue-agent lock exists in ${cfg.workdir}; remove .issue-agent.lock if the previous run died`,
        "warning",
      );
      return;
    }
  ...
  running = true;
  stopRequested = false;
  await ensureStandardLabels(cfg).catch((err) =>
    notify(
      `issue-agent label setup failed: ${err instanceof Error ? err.message : String(err)}`,
      "warning",
    ),
  );
  ...
  notify(
    cfg.dryRun
      ? `issue-agent dry run; printing operations without changing issues, git, or model state; ${argsSummary}`
      : `issue-agent running with workspace-permissions=deny (/tmp allowed); use /issue-agent-stop or Ctrl-C to stop after the current agent turn; ${argsSummary}`,
    "info",
  );
  try {
    while (!stopRequested) {
      if (!activeAgentRun) {
        ...
        await maintainIssueLabels(cfg, notify).catch((err) =>
          notify(
            `issue-agent label maintenance failed: ${err instanceof Error ? err.message : String(err)}`,
            "warning",
          ),
        );
        ...
        for (const repo of cfg.repos) {
          try {
            ...
            for (const pr of await listPullRequests(repo, cfg.token)) {
              ...
              if (stateOf(pr.labels) === "REQUESTING_REVIEW" || forced)
                prCandidates.push({ repo, pr, forced });
            }
          } catch (err) {
            notify(
              `issue-agent failed to list ${repo}: ${err instanceof Error ? err.message : String(err)}`,
              "warning",
            );
          }
        }
        const now = Date.now();
        prCandidates.sort(
          (a, b) => priorityOf(a.pr.labels) - priorityOf(b.pr.labels),
        );
        ...
        candidates.sort(
          (a, b) => priorityOf(a.issue.labels) - priorityOf(b.issue.labels),
        );
        const next = candidates.find((x) => {
          ...
          const anyModelAvailable =
            allModels.length === 0 ||
            allModels.some((m) => (usageLimitedUntil.get(m) ?? 0) <= now);
          return (
            ["PLANNING", "EXECUTING"].includes(state) &&
            now - queuedAt > QUEUED_COOLDOWN_MS &&
            !x.issue.labels.some((l) => /blocked/i.test(l)) &&
            !dependencyBlocked.get(`${x.repo}#${x.issue.number}`) &&
            anyModelAvailable
          );
        });
        if (nextPr) {
          ...
          try {
            if (
              nextPr.forced &&
              nextPr.pr.labels.includes(NO_TOOLCALL_LABEL) &&
              !cfg.dryRun
            )
              await removeLabel(
                nextPr.repo,
            ...
            recentlyQueued.set(
              `${nextPr.repo}#${nextPr.pr.number}:AI-REVIEW:${reviewCycle(nextPr.pr.labels)}:${nextPr.forced ? "forced" : "auto"}`,
              now,
            );
            notify(
              `Processed AI review for PR #${nextPr.pr.number} from ${basename(nextPr.repo)} in ${dir}`,
              "info",
            );
          } catch (err) {
            const message = err instanceof Error ? err.message : String(err);
            notify(
              `issue-agent AI-review failed for PR #${nextPr.pr.number}: ${message}`,
              "error",
            );
            activeAgentRun = false;
            activeWork = undefined;
          ...
          try {
            ...
            if (stopRequested) {
              notify(
                `issue-agent sub-agent stopped for #${next.issue.number}: ${message}`,
                "info",
              );
              activeAgentRun = false;
              activeWork = undefined;
            ...
            const providerLimited = /\b(429|rate.?limit|usage.?limit)\b/i.test(
              message,
            );
            const missingToolCall = /^missing required \w+ tool marker/i.test(
              message,
            );
            notify(
              `issue-agent sub-agent failed for #${next.issue.number}: ${message}`,
              "error",
            );
            if (!cfg.dryRun && !missingToolCall) {
              await postIssueComment(
                next.repo,
                next.issue,
                `Harness sub-agent failed while working this issue:\n\n${message}`,
                cfg.token,
              ).catch(() => undefined);
              await addLabels(
                next.repo,
          ...
          recentlyQueued.set(
            `${next.repo}#${next.issue.number}:${stateOf(next.issue.labels)}`,
            now,
          );
          setStatus(
            activeAgentRun
              ? `agent turn running for #${next.issue.number}`
              : `finished #${next.issue.number}`,
          );
          notify(
            `${activeAgentRun ? "Queued" : "Processed"} issue #${next.issue.number} from ${basename(next.repo)} in ${dir}`,
            "info",
          );
          if (cfg.dryRun && activeWork) {
            ...
            activeWork = undefined;
            if (stateOf(work.issue.labels) !== "PLANNING")
              work.doneText = "DRY RUN: assumed issueAgentDone PR text.";
            await finishWork(
              work,
              stateOf(work.issue.labels) === "PLANNING"
                ? "DRY RUN PLAN: assumed model plan output."
                : "DRY RUN: assumed issueAgentDone tool call.",
              notify,
            );
          }
        } else {
          setStatus(
            `idle; no runnable issue found, next poll in ${Math.round(cfg.intervalMs / 1000)}s`,
          );
        }
      }
...
export const __issueAgentTest = {
  runSubAgent,
  handleSubAgentJsonLine,
  parseArgs,
  isTruthy,
  labelValue,
  stateOf,
...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("after_provider_response", async (event, ctx) => {
    ...
    const fallbacks = models.fallback.filter(
      (m) => (usageLimitedUntil.get(m) ?? 0) <= Date.now(),
    );
    const next = fallbacks[activeWork.fallbackIndex + 1];
    const retry = event.headers["retry-after"]
      ? ` Retry-After: ${event.headers["retry-after"]}.`
      : "";
    const currentModel = String(
      (ctx as any).model?.provider && (ctx as any).model?.id
        ? `${(ctx as any).model.provider}/${(ctx as any).model.id}`
        : ((ctx as any).model?.id ?? "current-model"),
    );
    ...
    if (next && (await switchModelByName(pi, ctx, next))) {
      ...
      await postIssueComment(
        activeWork.repo,
        activeWork.issue,
        `Provider/model error HTTP ${event.status}.${retry}\n\nSwitching to fallback model \`${next}\` and retrying this issue.`,
        activeWork.cfg.token,
      ).catch(() => undefined);
      const work = activeWork;
      activeAgentRun = false;
    ...
    await postIssueComment(
      activeWork.repo,
      activeWork.issue,
      `Provider/model error while working this issue: HTTP ${event.status}.${retry}\n\nAll configured fallback models are exhausted or currently rate-limited. The harness will reschedule this issue after the limit resets and continue with other issues.`,
      activeWork.cfg.token,
    ).catch(() => undefined);
    await addLabels(
      activeWork.repo,
      ...
      [
        `ai:blocked/usage-limit`,
        `ai:provider-status/${event.status}`,
        `ai:retry-after/${new Date(retryAt).toISOString()}`,
      ],
      activeWork.cfg.token,
    ).catch(() => undefined);
    activeAgentRun = false;
  ...
  pi.on("agent_end", async (event) => {
    ...
    try {
      ...
      await postIssueComment(
        work.repo,
        work.issue,
        `Harness post-processing failed: ${err instanceof Error ? err.message : String(err)}`,
        work.cfg.token,
      ).catch(() => undefined);
      await addLabels(
        work.repo,
  ...
  pi.registerCommand("issue-agent", {
    ...
    handler: async (argLine: string, ctx) => {
      ...
      if (running) {
        ctx.ui.notify(
          "issue-agent is already running; use /issue-agent-stop first",
          "warning",
        );
        return;
      }
      const missingModels = configuredModelNames(cfg).filter(
        (model) => !findModelByName(ctx, model),
      );
      if (missingModels.length) {
        ctx.ui.notify(
          `Configured issue-agent model(s) not found: ${missingModels.map(normalizedModelName).join(", ")}. Not starting issue-agent.`,
          "warning",
        );
        return;
      }
      ...
      if (ctx.hasUI) {
        ...
        const paint = () => {
          const text = running
            ? `${frames[frame++ % frames.length]} ${formatStatus()}`
            : undefined;
          ctx.ui.setStatus("issue-agent", text);
          ctx.ui.setWidget(
            "issue-agent",
            text ? [`issue-agent ${text}`] : undefined,
        ...
        stopStatusUi = () => {
          clearInterval(timer);
          ctx.ui.setStatus("issue-agent", undefined);
          ctx.ui.setWidget("issue-agent", undefined);
        };
      }
      try {
  ...
  pi.registerCommand("issue-agent-status", {
    description:
      "Show what /issue-agent is currently doing and where to view the spawned task chat.",
    handler: async (_argLine: string, ctx) => {
      ctx.ui.notify(formatStatus(), running ? "info" : "warning");
      if (activeWork) {
        ctx.ui.notify(
          "The issue is being handled by an isolated sub-agent process in the checkout; its stdout/stderr is piped back into this chat as follow-up issue-agent messages.",
          "info",
        );
      }
    },
  ...
  pi.registerCommand("issue-agent-stop", {
    description:
      "Gracefully stop the long-running /issue-agent loop after the current agent turn.",
    handler: async (_argLine: string, ctx) => {
      if (!running) {
        ctx.ui.notify("issue-agent is not running", "info");
        return;
      }
      stopRequested = true;
      stopActiveSubAgent();
      ctx.ui.notify(
        "issue-agent stop requested; active sub-agent is being terminated",
        "info",
      );
    },
  });
  ...
  if (process.env.ISSUE_AGENT_DONE_FILE) {
    pi.registerTool({
      ...
      description:
        "Mark the active issue-agent execution task as done. Optional text is used in the issue completion comment and pull request body.",
      parameters: Type.Object({ text: Type.Optional(Type.String()) }),
      async execute(_id, input: any) {
        const text = typeof input.text === "string" ? input.text : "";
        writeFileSync(
          process.env.ISSUE_AGENT_DONE_FILE!,
          JSON.stringify({ text }, null, 2),
        );
        return {
          content: [
            { type: "text", text: "Marked sub-agent issue task as done." },
          ],
          details: { ok: true },
        };
      },
  ...
  pi.registerTool({
    ...
    description:
      "Ask the issue reporter for clarification during issue-agent planning without blocking. Provide question(s), optional choices, and context needed to resume after /answer.",
    ...
    async execute(_id, input: any) {
      ...
      if (!ask.question && !ask.questions?.length)
        return {
          content: [
            {
              type: "text",
              text: "issueAgentAsk requires question or questions.",
            },
          ],
          details: { ok: false },
        };
      if (!ask.context.trim())
        return {
          content: [
            {
              type: "text",
              text: "issueAgentAsk requires context for resuming the planning agent.",
            },
          ],
          details: { ok: false },
        };
      if (process.env.ISSUE_AGENT_DONE_FILE) {
        ...
        return {
          content: [
            {
              type: "text",
              text: "Recorded issue-agent clarification request.",
            },
          ],
          details: { ok: true },
        };
      }
      if (!activeWork || stateOf(activeWork.issue.labels) !== "PLANNING")
        return {
          content: [
            { type: "text", text: "No active issue-agent planning task." },
          ],
          details: { ok: false },
        };
      ...
      return {
        content: [
          { type: "text", text: "Recorded issue-agent clarification request." },
        ],
        details: { ok: true },
      };
    },
  ...
  pi.registerTool({
    ...
    description:
      "List ai:* labeled GitHub/Forgejo issues by priority/state without starting work.",
    parameters: Type.Object({
      repos: Type.String(),
    ...
    async execute(_id, input: any) {
      ...
      for (const repo of repos)
        for (const issue of await listIssues(repo, input.token))
          rows.push({ repo, issue });
      rows.sort(
        (a, b) => priorityOf(a.issue.labels) - priorityOf(b.issue.labels),
      );
      return {
        content: [
          {
            ...
            text:
              rows
                .map(
                  (r) =>
                    `${priorityOf(r.issue.labels)} ${stateOf(r.issue.labels)} ${r.repo}#${r.issue.number} ${r.issue.title}`,
                )
                .join("\n") || "No ai issues found",
          },
        ],
