...
describe("issue-agent lifecycle helpers", () => {
  it("parses command args, truthy flags, and ai label syntaxes", () => {
    expect(
      __issueAgentTest.parseArgs(
        '--repos "https://github.com/a/b" --dry-run --interval=50',
      ),
    ).toEqual({
    ...
    expect(
      __issueAgentTest.labelValue(
        ["ai:priority/5", "ai[planning-model:openai/o3]"],
        "priority",
      ),
    ).toBe("5");
    expect(
  ...
  it("parses REQUESTING_REVIEW state labels and priority defaults", () => {
    expect(__issueAgentTest.stateOf(["ai:state/REQUESTING_REVIEW"])).toBe(
      "REQUESTING_REVIEW",
    );
    expect(__issueAgentTest.stateOf(["ai[state:REQUESTING_REVIEW]"])).toBe(
      "REQUESTING_REVIEW",
  ...
  it("derives model, thinking, dependency, branch, and autoresearch metadata", () => {
    const labels = [
      "ai:planning-model/plan-model",
      "ai:execution-model:exec-model",
      "ai:fallback-small-model/fallback-a",
      "ai[fallback-large-model:fallback-b]",
      "ai:thinking-level/high",
    ];
    expect(__issueAgentTest.modelsOf(labels, { fallback: [] })).toEqual({
      planning: "plan-model",
      execution: "exec-model",
      fallback: ["fallback-a", "fallback-b"],
    });
    expect(
      __issueAgentTest.modelsOf(labels, {
        planning: "override",
    ...
    expect(
      __issueAgentTest.thinkingLevelOf(["ai:thinking-level/banana"]),
    ).toBeUndefined();
    const issue = {
      number: 7,
      title: "Fix: Thing!!!",
    ...
    expect(
      __issueAgentTest.isAutoresearch({
        ...issue,
        labels: ["ai:autoresearch"],
      }),
    ).toBe(true);
    expect(
      __issueAgentTest.autoresearchConfig({
        ...issue,
        labels: [
          "autoresearch:max-iterations=3",
          "autoresearch:metric=accuracy",
          "autoresearch:direction=max",
        ],
      }),
    ).toEqual({ maxIterations: "3", metric: "accuracy", direction: "max" });
  });

  it("normalizes pull request rows with branch/base metadata", () => {
    const pr = __issueAgentTest.normalizePullRequest({
      number: 7,
      title: "T",
      labels: [{ name: "ai:state/REQUESTING_REVIEW" }],
      html_url: "h",
      issue_url: "i",
      url: "p",
      head: { label: "owner:feature", repo: { html_url: "not-a-clone-url" } },
      base: { ref: "main" },
    });
  ...
  it("detects slash review comments and review cycles", () => {
    expect(
      __issueAgentTest.hasReviewCommand([{ body: "please\n/review" }]),
    ).toBe(true);
    expect(
      __issueAgentTest.hasUnansweredReviewCommand([
        { body: "/review", created_at: "2026-01-01T00:00:00Z" },
        { body: "## AI Review Summary", created_at: "2026-01-01T00:01:00Z" },
      ]),
    ).toBe(false);
    expect(
      __issueAgentTest.hasUnansweredReviewCommand([
        { body: "/review", created_at: "2026-01-01T00:00:00Z" },
        { body: "## AI Review Summary", created_at: "2026-01-01T00:00:00Z" },
      ]),
    ).toBe(false);
    expect(
      __issueAgentTest.hasUnansweredReviewCommand([
    ...
    expect(
      __issueAgentTest.hasUnansweredReviewCommand([
        { body: "## AI Review Summary", created_at: "2026-01-01T00:00:00Z" },
        { body: "/review", created_at: "2026-01-01T00:01:00Z" },
      ]),
    ).toBe(true);
    expect(
      __issueAgentTest.hasUnansweredReviewCommand([
  ...
  it("constructs platform-specific PR review events", () => {
    ...
    expect(__issueAgentTest.reviewEvent("request_changes")).toBe(
      "REQUEST_CHANGES",
    );
  });
  ...
  it("round-trips issue-agent ask comments and /answer comments", () => {
    const ask = {
      question: "Which API?",
      choices: ["A", "B"],
      context: "Found callers in src/api.ts",
    };
    ...
    expect(
      __issueAgentTest.latestIssueAgentAnswer([
        { body: askComment },
        { body: "/answer Use A" },
        { body: "## AI Plan" },
      ]),
  ...
  it("matches the latest /answer with the most recent unanswered clarification", () => {
    const first = __issueAgentTest.formatIssueAgentAskComment({
      question: "First?",
      context: "first context",
    });
    const second = __issueAgentTest.formatIssueAgentAskComment({
      questions: ["Second?", "More detail?"],
  ...
  it("handles sub-agent json tool events, errors, and assistant content", () => {
    ...
    const emit = (msg: string, type?: "info" | "warning" | "error") =>
      emitted.push({ msg, type });
    expect(
      __issueAgentTest.handleSubAgentJsonLine(
    ...
    expect(
      __issueAgentTest.handleSubAgentJsonLine(
        JSON.stringify({
          message: { stopReason: "error", errorMessage: "bad" },
        }),
        emit,
      ),
    ).toBe("bad");
    expect(
      __issueAgentTest.handleSubAgentJsonLine("not json", emit),
    ).toBeUndefined();
  ...
  it("issue planning prompt includes shared guidance and issueAgentAsk", () => {
    const prompt = __issueAgentTest.issuePrompt(
      "PLANNING",
      "https://github.com/acme/repo",
      { number: 1, title: "T", body: "B", labels: [], url: "u", apiUrl: "a" },
      "/tmp/repo",
      { fallback: [] },
      undefined,
      "",
  ...
  it("repairs discovered PR issue API URLs even when PR rows already include labels", async () => {
    ...
    global.fetch = vi.fn(async (url: any) => {
      expect(String(url)).toBe(
        "https://api.github.com/repos/acme/repo/pulls?state=open&per_page=100",
      );
      return new Response(
        JSON.stringify([
          {
    ...
    try {
      const prs = await __issueAgentTest.listPullRequests(
        "https://github.com/acme/repo.git",
        "tok",
      );
      expect(prs).toHaveLength(1);
      expect(prs[0].apiUrl).toBe(
  ...
  it("uses PR labels as authoritative after PR creation and marks the issue PR_PENDING", async () => {
    ...
    try {
      const issue = {
        ...
        title: "Test",
        body: "",
        labels: ["ai:state/EXECUTING"],
        url: "issue-html",
        apiUrl: "https://github.com/api-issue",
      };
      const pr = await __issueAgentTest.createPullRequest(
        "https://github.com/acme/repo.git",
        issue,
        "ai/1-test",
        "tok",
      ...
      await __issueAgentTest.addLabels(
        "https://github.com/acme/repo.git",
        pr,
        ["ai:source/AGENT", __issueAgentTest.stateLabel("REQUESTING_REVIEW")],
        "tok",
      );
      await __issueAgentTest.setAiState(
        "https://github.com/acme/repo.git",
        issue,
        "PR_PENDING",
        "tok",
    ...
    expect(
      calls.some(
        (c) => c.url === "https://api.github.com/repos/acme/repo/pulls",
      ),
    ).toBe(true);
    expect(
      calls.some(
        (c) =>
          c.url === "https://api.github.com/repos/acme/repo/issues/9/labels" &&
          String(c.init?.body).includes("REQUESTING_REVIEW"),
      ),
    ).toBe(true);
    expect(
      calls.some(
        (c) =>
          c.url ===
            "https://github.com/api-issue/labels/ai%3Astate%2FEXECUTING" &&
    ...
    expect(
      calls.some(
        (c) =>
          c.url === "https://github.com/api-issue/labels" &&
          String(c.init?.body).includes("PR_PENDING"),
      ),
    ).toBe(true);
  });
});
...
describe("issue-agent runSubAgent", () => {
  ...
  it("resolves with a completed plan from the sub-agent", async () => {
    process.env.ISSUE_AGENT_LITTLE_CODER_BIN = script(
      tmp,
      `console.log(${JSON.stringify(assistantLine("# PLAN\nDo it"))});`,
    );
    const emitted: string[] = [];
    const out = await __issueAgentTest.runSubAgent(
      "prompt",
      tmp,
      tmp,
      undefined,
      undefined,
      (msg: string) => emitted.push(msg),
    );
    expect(out).toBe("# PLAN\nDo it");
    expect(
      emitted.filter((x) => x.includes("sub-agent assistant")),
    ).toHaveLength(1);
  });
  ...
  it("deduplicates repeated assistant snapshots", async () => {
    ...
    process.env.ISSUE_AGENT_LITTLE_CODER_BIN = script(
      tmp,
      `for (let i = 0; i < 4; i++) console.log(${JSON.stringify(line)});`,
    );
    const emitted: string[] = [];
    const out = await __issueAgentTest.runSubAgent(
      "prompt",
  ...
  it("ignores non-json stdout noise and still parses json", async () => {
    process.env.ISSUE_AGENT_LITTLE_CODER_BIN = script(
      tmp,
      `console.log('> little-coder banner'); console.log(${JSON.stringify(assistantLine("valid"))});`,
    );
    const emitted: string[] = [];
    const out = await __issueAgentTest.runSubAgent(
      "prompt",
  ...
  it("reports stderr on non-zero exit", async () => {
    ...
    expect(emitted.some((x) => x.includes("sub-agent stderr: boom"))).toBe(
      true,
    );
  });
