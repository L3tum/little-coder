...
const inspectPackageRoot = resolvePath(
  fileURLToPath(new URL("../../../node_modules/pi-inspect/", import.meta.url)),
);
...
function readIndex(): IndexEntry[] {
  try {
    ...
    if (e?.code !== "ENOENT")
      console.warn(`pi-inspect: read index: ${e.message}`);
    return [];
  }
}
...
function writeSnapshot(id: string, snapshot: unknown): void {
  mkdirSync(SNAP_DIR, { recursive: true });
  writeFileSync(
    joinPath(SNAP_DIR, `${sanitize(id)}.json`),
...
const KIND_MAP: Record<
  ResourceKind,
  { kind: DisabledKind; display: (n: string) => string }
> = {
...
function buildItemFromResource(
  ...
  return {
    ...
    settingsPath:
      scope === "project"
        ? joinPath(sessionCwd, ".pi", "settings.json")
        : SETTINGS_PATH,
    path: r.path,
    reason: md.source,
    resourceKind: kind,
...
async function discoverFromPackages(
  ...
  return {
    disabled: [...disabledByPath.values()],
...
async function processToggleRequest(
  req: ToggleRequest,
  sessionCwd: string,
): Promise<void> {
  const sm = SettingsManager.create(sessionCwd, AGENT_DIR);
  ...
  if (md.origin === "package") {
    ...
    const idx = packages.findIndex(
      (p) => (typeof p === "string" ? p : p.source) === md.source,
    );
    ...
    const arrayKey = req.resourceKind as keyof typeof pkg &
  ...
  const settings =
    scope === "project" ? sm.getProjectSettings() : sm.getGlobalSettings();
  const current =
    ((settings as any)[req.resourceKind] as string[] | undefined) ?? [];
  const updated = applyPattern(current, pattern, enabled);
  const setters: Record<ResourceKind, { user: string; project: string }> = {
    skills: { user: "setSkillPaths", project: "setProjectSkillPaths" },
    prompts: {
      user: "setPromptTemplatePaths",
      project: "setProjectPromptTemplatePaths",
    },
    extensions: {
      user: "setExtensionPaths",
      project: "setProjectExtensionPaths",
    },
...
async function handleRequestFile(full: string): Promise<void> {
  ...
  try {
    req = JSON.parse(raw) as ToggleRequest;
  } catch (e: any) {
    console.warn(`pi-inspect: bad request file ${full}: ${e?.message ?? e}`);
    try {
      renameSync(full, `${full}.err.json`);
    } catch {}
    return;
  }
  ...
  try {
    ...
    try {
      writeFileSync(
        `${full}.err.json`,
        JSON.stringify({ error: e?.message ?? String(e), req }, null, 2),
      );
      unlinkSync(full);
    } catch {}
  }
...
async function captureSnapshot(
  ctx: any,
  extra: { systemPromptOptions?: any; providerPayload?: any } = {},
): Promise<{ id: string; entry: IndexEntry } | null> {
  ...
  const systemPrompt =
    typeof ctx.getSystemPrompt === "function" ? ctx.getSystemPrompt() : null;
  const sessionEntries =
    typeof sm?.getEntries === "function" ? sm.getEntries() : null;
  const sessionState = stateForSession(id);
  if (extra.systemPromptOptions !== undefined)
    sessionState.systemPromptOptions = extra.systemPromptOptions;
  if (extra.providerPayload !== undefined)
    sessionState.providerPayload = extra.providerPayload;
  ...
  for (const c of commands) {
    ...
    if (typeof p === "string")
      activePaths.add(p.replace(/\\/g, "/").toLowerCase());
  }
  ...
  const { disabled: disabledItems, pending: pendingItems } =
    await discoverFromPackages(cwd, activePaths);
  ...
  pi.registerCommand("inspect", {
    ...
    getArgumentCompletions: (prefix) => {
      ...
      if (tokens.length >= 1 && tokens[0] === "open") {
        const v = tokens[1] ?? "";
        return OPEN_TARGETS.filter((t) => t.startsWith(v)).map((t) => ({
          value: `open ${t}`,
          label: t,
        }));
      }
    ...
    handler: async (args, ctx) => {
      lastCtx = ctx;
      const notify = (m: string, l: "info" | "error" = "info") =>
        ctx.ui.notify(m, l);
      ...
      if (first === "list") {
        ...
        if (!list.length) {
          notify("no snapshots yet — run /inspect to capture this session");
          return;
        }
      ...
      if (first === "snapshot") {
        ...
        notify(
          r ? `snapshot captured: ${r.id}` : "no active session to snapshot",
          r ? "info" : "error",
        );
        return;
      }
      ...
      const openTarget = (isExplicitOpen ? (tokens[1] ?? "web") : "web") as
        "web" | "app";
      ...
      if (
        !isExplicitOpen &&
        first &&
        !(SUBCOMMANDS as readonly string[]).includes(first)
      ) {
      ...
      const target = openId
        ? `${url}/?session=${encodeURIComponent(openId)}`
        : url;
      ...
      if (!isExplicitOpen) {
        try {
          ...
          if (delivered > 0) {
            notify(
              `focused existing dashboard → session ${openId ?? "(latest)"}`,
            );
            return;
          }
        } catch {}
