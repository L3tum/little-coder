  ...
  while (current.length >= stop.length) {
    try {
      if (
        fs
          .readdirSync(current)
          .some((name) => extensions.some((ext) => name.endsWith(ext)))
      )
        return current;
    } catch {}
    const parent = path.dirname(current);
...
function findRootCsharp(file: string, cwd: string): string | undefined {
  return (
    findRoot(file, cwd, ["global.json"]) ??
    findRootByExtension(file, cwd, [".sln", ".csproj"])
  );
}
  ...
  },
  {
    ...
    spawn: async (root) => {
      const cmd =
        ...
        which("Microsoft.CodeAnalysis.LanguageServer.dll") ??
        which("roslyn-lsp");
      if (!cmd) return undefined;
