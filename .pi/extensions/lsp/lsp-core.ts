...
function findRootByExtension(file: string, cwd: string, extensions: string[]): string | undefined {
  let current = path.resolve(path.dirname(file));
  const stop = path.resolve(cwd);
  while (current.length >= stop.length) {
    try {
      if (fs.readdirSync(current).some((name) => extensions.some((ext) => name.endsWith(ext)))) return current;
    } catch {}
    const parent = path.dirname(current);
    if (parent === current) break;
    current = parent;
...
function findRootCsharp(file: string, cwd: string): string | undefined {
  return findRoot(file, cwd, ["global.json"]) ?? findRootByExtension(file, cwd, [".sln", ".csproj"]);
}
...
export const LSP_SERVERS: LSPServerConfig[] = [
  ...
  { id: "rust-analyzer", extensions: [".rs"], findRoot: (f, cwd) => findRoot(f, cwd, ["Cargo.toml"]), spawn: simpleSpawn("rust-analyzer", []) },
  {
    id: "csharp-lsp",
    extensions: [".cs"],
    findRoot: (f, cwd) => findRootCsharp(f, cwd),
    spawn: async (root) => {
      const cmd = which("csharp-ls") ?? which("csharp-lsp");
      if (!cmd) return undefined;
      return { process: spawn(cmd, ["--stdio"], { cwd: root, stdio: ["pipe", "pipe", "pipe"] }) };
    },
  },
  {
    ...
    findRoot: (f, cwd) => findRootCsharp(f, cwd),
    spawn: async (root) => {
      const cmd = which("Microsoft.CodeAnalysis.LanguageServer") ?? which("Microsoft.CodeAnalysis.LanguageServer.dll") ?? which("roslyn-lsp");
      if (!cmd) return undefined;
      const args = cmd.endsWith(".dll") ? [cmd, "--stdio"] : ["--stdio"];
      const bin = cmd.endsWith(".dll") ? (which("dotnet") ?? "dotnet") : cmd;
      return { process: spawn(bin, args, { cwd: root, stdio: ["pipe", "pipe", "pipe"] }) };
    },
  },
];
