...
function renderToolsListing(pi: ExtensionAPI): string {
  ...
  const hiddenBrowser = BROWSER_TOOL_NAMES.filter(
  ...
  if (hiddenBrowser.length > 0) {
    ...
    lines.push(
      "  Use enableBrowserTools (tool) or /enable-browser-tools (command) to load them.",
    );
  }
  lines.push("");
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    label: "Tools",
    description:
      "List the current tool registry, including Browser* tools that are discoverable on demand.",
    promptSnippet:
      "tools(): list currently loaded tools and on-demand Browser* tools.",
    parameters: Type.Object({}),
    async execute() {
      return {
        content: [{ type: "text", text: renderToolsListing(pi) }],
        details: {},
      };
    },
  });
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      ignoreDefaultExcludes: Type.Optional(
        Type.Boolean({
  ...
  pi.registerTool({
    ...
    promptSnippet:
      "findRead(pattern, path?, maxFiles?, maxCharacters?, ignoreDefaultExcludes?): glob + read in one call.",
    parameters: Type.Object({
      pattern: Type.String({ description: "Glob pattern e.g. **/*.py" }),
      path: Type.Optional(
        Type.String({ description: "Base directory (default: cwd)" }),
      ...
      maxFiles: Type.Optional(
        Type.Number({ description: "Max files to read (default 10, max 50)" }),
      ),
      maxCharacters: Type.Optional(
        Type.Number({
          description: "Max characters per file (default 4000, 0 = unlimited)",
        }),
      ),
      ignoreDefaultExcludes: Type.Optional(
        Type.Boolean({
    ...
    prepareArguments(args) {
      ...
      return {
        ...
        maxFiles:
          typeof input.maxFiles === "number" ? input.maxFiles : input.max_files,
        maxCharacters,
        ignoreDefaultExcludes:
          typeof input.ignoreDefaultExcludes === "boolean"
    ...
    async execute(
      _id,
      { pattern, path, maxFiles, maxCharacters, ignoreDefaultExcludes },
    ): Promise<any> {
      const base = path || process.cwd();
      ...
      try {
        ...
        if (matches.length === 0) {
          return {
            content: [
              { type: "text", text: invocation + renderGlobOutcome(outcome) },
            ],
            details: { filesRead: 0, totalMatched: 0 },
          };
        }
        ...
        if (outcome.scanTruncated) {
          suffix.push(
            `[Search stopped after scanning many entries; results may be incomplete. Narrow the base path.]`,
          );
        }
        if (truncated.length > 0) {
          suffix.push(
            `[Truncated to ${charLimit} characters: ${truncated.join(", ")}]`,
          );
        }
        ...
        return {
          content: [
            {
              type: "text",
              text: invocation + `Error: ${(e as Error).message}`,
            },
          ],
