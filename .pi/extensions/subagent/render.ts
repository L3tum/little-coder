...
/** Minimal theme shape expected by the subagent render pipeline. */
...
function formatToolCall(
  ...
  switch (toolName) {
    ...
    case "ls":
      return (
        fg("muted", "ls ") +
        fg("accent", shortenPath((args.path || ".") as string))
      );
    case "find":
      return (
        fg("muted", "find ") +
    ...
    default:
      return (
        fg("accent", toolName) +
        fg("dim", ` ${truncate(JSON.stringify(args), 50)}`)
      );
  }
}
...
export function renderCall(
  ...
  _context?: { state: Record<string, unknown>; invalidate: () => void },
): Text {
...
function renderParallelExpanded(
  ...
  for (const r of details.results) {
    ...
    if (finalOutput) {
      ...
      container.addChild(new Spacer(1));
      container.addChild(
        new Text(theme.fg("error", getResultSummaryText(r)), 0, 0),
      );
    }
