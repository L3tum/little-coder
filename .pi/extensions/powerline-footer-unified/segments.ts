...
const costSegment: StatusLineSegment = {
  ...
  render(ctx) {
    ...
    if (usingSubscription) {
      const usage = ctx.openaiUsageSnapshot;
      const parts = [`${withIcon(icons.cost, cost.toFixed(3))} (sub)`];
      if (usage) {
        parts.push(`5h ${formatPercent(usage.fiveHourLeftPercent)}`);
        parts.push(`7d ${formatPercent(usage.sevenDayLeftPercent)}`);
        const fiveHourReset = formatReset(usage.fiveHourResetInSeconds);
        const sevenDayReset = formatReset(usage.sevenDayResetInSeconds);
        if (fiveHourReset) parts.push(`5h reset ${fiveHourReset}`);
        if (sevenDayReset) parts.push(`7d reset ${sevenDayReset}`);
        if (usage.isLimited) parts.push("limited");
      }
      return {
        content: color(ctx, "cost", parts.join(SEP_DOT)),
        visible: true,
      };
    }

    const costDisplay = withIcon(icons.cost, cost.toFixed(2));
