...
const OPENAI_USAGE_REFRESH_MS = 60_000;
...
export default function powerlineFooter(pi: ExtensionAPI) {
  ...
  const refreshOpenAIUsage = async (ctx: any, modelId = ctx.model?.id) => {
    if (!isOpenAISubscriptionModel(ctx)) {
      openaiUsageSnapshot = undefined;
      requestImmediateStatusRender();
      return;
    }

    openaiUsageAbortController?.abort();
    const controller = new AbortController();
    openaiUsageAbortController = controller;
    try {
      const usage = await requestCodexUsage(controller.signal);
      if (usage) {
        openaiUsageSnapshot = parseUsageSnapshot(usage, modelId);
      }
    } catch {
      // Keep the previous snapshot; the next timer/turn will retry.
    } finally {
      if (openaiUsageAbortController === controller) {
        openaiUsageAbortController = undefined;
      }
      requestImmediateStatusRender();
    }
  };
  ...
  pi.on("model_select", async (event, ctx) => {
    currentCtx = ctx;
    openaiUsageSnapshot = undefined;
    void refreshOpenAIUsage(ctx, event.model.id);
    requestStatusRender();
  });
