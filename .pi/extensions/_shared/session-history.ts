...
function eventText(obj: any): string {
  const c =
    obj?.content ?? obj?.message?.content ?? obj?.text ?? obj?.prompt ?? "";
...
export function parseSessionFile(path: string): SessionOutline | undefined {
  try {
    ...
    return {
      id: relative(agentDir(), path).replace(/\.jsonl$/, ""),
      path,
      cwd,
      project,
      date,
...
export function discoverSessions(base = agentDir()): SessionOutline[] {
  ...
  return walkJsonl(dir)
    .map(parseSessionFile)
    .filter((x): x is SessionOutline => !!x)
    .sort((a, b) => (b.date || "").localeCompare(a.date || ""));
}

export function outlineText(session: SessionOutline, maxTurns = 8): string {
  return session.turns
    .filter((t) => t.text && t.role !== "tool_result")
    .slice(-maxTurns)
    .map(
      (t) =>
        `${t.role}${t.toolName ? `:${t.toolName}` : ""}: ${t.text.slice(0, 300)}`,
    )
    .join("\n");
}
...
export function lexicalSessionScore(
  query: string,
  ...
  for (const term of terms) {
    ...
    for (const turn of session.turns) {
      ...
      if (hits === 0) continue;
      const roleBoost =
        turn.role === "user" ? 3 : turn.role === "tool" ? 1.5 : 1;
      const pathBoost = /(?:^|[\s"'`])(?:\.?\.?\/)?[\w.-]+(?:\/[\w.-]+)+/.test(
        turn.text,
      )
        ? 1.5
        : 1;
...
export function searchSessions(
  ...
  cwd = process.cwd(),
  limit = 5,
): Array<SessionOutline & { score: number; snippet: string; mode: string }> {
  const terms = query.toLowerCase().split(/\W+/).filter(Boolean);
  return sessions
    .map((s) => {
...
export async function searchSessionsWithMode(
  query: string,
  mode = "lexical",
  sessions = discoverSessions(),
  cwd = process.cwd(),
  ...
  rows: Array<
    SessionOutline & { score: number; snippet: string; mode: string }
  >;
  mode: string;
  note?: string;
}> {
  if (mode !== "semantic")
    return {
      rows: searchSessions(query, sessions, cwd, limit),
      mode: "lexical",
    };
  try {
    await import("@tobilu/qmd" as any);
    return {
      rows: searchSessions(query, sessions, cwd, limit).map((r) => ({
        ...r,
        mode: "semantic-fallback",
      })),
      mode: "lexical-fallback",
      note: "QMD is installed, but session semantic indexing is not built yet; using lexical fallback.",
    };
  } catch {
    return {
      rows: searchSessions(query, sessions, cwd, limit),
      mode: "lexical-fallback",
      note: "Semantic session search is unavailable because @tobilu/qmd could not initialize; using lexical fallback.",
    };
  }
}
