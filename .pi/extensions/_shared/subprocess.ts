...
export type ManagedSubprocessStatus =
  "running" | "exited" | "error" | "stopping";
...
export function startSubprocess(
  command: string,
  args: readonly string[] = [],
  options: StartSubprocessOptions = {},
): ManagedSubprocess {
  const { name, ...spawnOptions } = options;
  const child = spawn(command, [...args], spawnOptions);
  return registerChildProcess(child, command, args, {
    name,
    cwd: spawnOptions.cwd,
  });
}
...
export function stopSubprocess(
  id: number,
...
/** Reset internal state. Intended for test cleanup only. */
