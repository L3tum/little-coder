...
export function normalizeEditArguments(args: unknown): unknown {
  ...
  const normalizeOne = (edit: unknown) => {
    ...
    const oldText = pickString(e, [
      "oldText",
      "old_string",
      "oldString",
      "old_text",
    ]);
    const newText = pickString(e, [
      "newText",
      "new_string",
      "newString",
      "new_text",
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    promptSnippet:
      "edit(path, edits): patch an existing file using exact old_string → new_string replacements.",
    parameters: Type.Object({
      path: Type.Optional(
        Type.String({ description: "Path of the file to edit" }),
      ),
        ...
        Type.Array(
          Type.Object({
            old_string: Type.Optional(
              Type.String({
                description:
                  "Exact text for one targeted replacement. It must be unique in the original file and must not overlap with any other edits[].old_string in the same call.",
              }),
            ),
          ...
          {
            description:
              "One or more targeted replacements. Each edit is matched against the original file, not incrementally. Alias keys oldString/old_text/oldText and newString/new_text/newText are accepted and normalized internally.",
          },
        ),
      ),
      edit: Type.Optional(
    ...
    renderCall(args: any, theme: any, context: any) {
      ...
      return base.renderCall?.(normalizedArgs, theme, {
        ...context,
        args: normalizedArgs,
      });
    },
    renderResult(result: any, options: any, theme: any, context: any) {
      ...
      return base.renderResult?.(result, options, theme, {
        ...context,
        args: normalizedArgs,
      });
    },
