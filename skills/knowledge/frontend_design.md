...
- **Motion**: Use animations for effects and micro-interactions. Prioritize CSS-only for HTML. Use Motion library for React. Focus on high-impact moments: staggered reveals, scroll-triggering, hover states that surprise.
...
NEVER use generic AI aesthetics: overused font families (Inter, Roboto, Arial, system fonts), cliched color schemes (purple gradients on white), predictable layouts, cookie-cutter design.

Match implementation complexity to the aesthetic vision: maximalist designs need elaborate code and effects; minimalist designs need restraint, precision, and careful attention to spacing and typography.
