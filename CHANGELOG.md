...
- **The programmatic pipelines (`/review`, `/review-project`, `/deep-plan`) now show a live progress panel.** Command handlers have no in-flight tool rendering (unlike the subagent tool), so a multi-phase run was previously silent apart from a footer status line. Each run now renders a widget above the editor — one row per phase with status icon, elapsed time, and a one-line “currently doing” tail derived from the phase's streaming output (current tool call, or “writing… (N chars)” while the final text streams). The panel updates in place (per-run widget key), throttles streaming re-renders to ≤2.5/s with a 1 s elapsed-time heartbeat, and is cleared on every exit path (success, per-phase failure, backstop). Headless/test contexts without `ui.setWidget` are no-ops, so pipeline behavior is unchanged there.

- **`max_tokens: 0` disables the output limit on local servers, and a profile's `max_tokens` now reaches the request.** A `before_provider_request` hook in `benchmark-profiles` injects the profile's `max_tokens` onto the outgoing request (previously it was display-only metadata, so the model catalog's `maxTokens` always won). `0` means "no output cap" **on local servers** (llama.cpp/ollama/lmstudio): the request carries a sentinel cap the server clamps to remaining context; on a remote provider `0` simply omits the cap (the catalog default applies — remote APIs reject the unlimited sentinel). Any positive value is capped by the model's registered context window so a bad value can't blow past the server's budget.
- **`little_coder.bash_allow` in the existing pi settings files.** Shell allow-prefixes can now live in `~/.pi/agent/settings.json` (per-user, always honored) or `<repo>/.pi/settings.json` (per-repo, honored only after the project is trusted — an untrusted repo's settings file can't widen the shell allowlist). Same word-boundary convention as `LITTLE_CODER_BASH_ALLOW`, merged additively with builtins + env; a one-time session notification names the active project-scoped prefixes (first six, then a count); malformed entries are ignored without failing startup.
- **`/allow <command>` and `/deny <command>`: per-repo shell allowlists.** `/allow` adds a word-boundary prefix (e.g. `make test` → `"make test "`) to the current repo's allowlist; `/deny` removes one. They write to the user's `~/.pi/agent/settings.json` by default — never into the repo's own `.pi/settings.json`, so a repo can't auto-approve its own shell commands and nothing is committed to the working tree. `bash_allow` in the user file is now polymorphic: a plain array (purely global, unchanged) or a per-repo map `{"global": [...], "/abs/repo/path": [...]}`; the map converts back to an array when no repo entries remain. Repo key is the canonical launch directory (realpath, matching `trust.json`). Changes apply immediately in the running session; a one-time notice names the active repo-scoped prefixes (first six, then a count). `/allow <command> --global` and `/deny <command> --global` write to the shared `"global"` key instead of the current repo's key, so the prefix is honored in every repo — because it lives in your own user settings file it is never trust-gated. `/deny` only edits settings-file entries — when a command is still allowed via builtins, `LITTLE_CODER_BASH_ALLOW`, or the global list, the reply names the source that keeps it allowed. The settings writer is fail-safe: a malformed `settings.json` is never clobbered.
- **Shared `little_coder` settings resolver.** `_shared/little-coder-settings.ts` resolves the `little_coder` namespace in package defaults → per-user agent-dir file → per-repo `.pi/settings.json` order, with `PI_CODING_AGENT_DIR`/`LITTLE_CODER_PKG_ROOT` test overrides, and re-reads the files on every use (the resolver itself is unmemoized — note the permission-gate bash allowlist caches its resolved union per session, applied on session start / after `/allow`|`/deny`). `benchmark-profiles` (profiles; project-scope profiles are trust-gated) and `permission-gate` (bash allowlist) both consume it, and `_shared/project-trust.ts` gives both one pi-faithful, fail-closed project-trust matrix (a read-only `trust.json` lookup — nearest ancestor decision, matching pi's `findNearestTrustEntry`).
- **Launch timing instrumentation.** `LITTLE_CODER_TIMING=1` prints launcher phase timings (discovery/updatecheck/settings/spawn/total), the child `--import` preload cost, and pi's own per-extension startup timings (via `PI_TIMING=1` passthrough); `docs/startup-performance.md` documents the breakdown and the profiling recipe.
...
- **Subagent transient remote/provider failures are retried, and real errors no longer hide behind stale partial output.** Previously a child whose LAST LLM call failed (rate limit, 5xx, dropped connection, stream cut short) was misclassified as a *success*: `hasSemanticCompletion` treated the stale assistant text + `agent_end` as completion, so the parent model saw whatever the child had written earlier (e.g. "The bash tool isn't available in this environment") as the subagent's answer and the real error never surfaced. A run whose final turn ended in `stopReason: "error"` is now always an error (`hasSemanticCompletion` gates on it), and error summaries lead with the ACTUAL error (`errorMessage` → `stderr`) with the stale partial output preserved below it as explicit "Partial output before failure:" context (both `getResultSummaryText` and the pipeline's `toPhaseOutcome` are error-first). On top of that, `runAgent` retries transient failures with exponential backoff + jitter (~30 s then ~60 s, ±25%, 3 total spawns — a ~1.5 min retry window sized so transient provider cooldowns have time to clear) using pi-ai's own `isRetryableAssistantError`/`isContextOverflow` classifiers, so "transient" means exactly what pi's in-session retries mean: explicit context overflow, quota/billing exhaustion, aborts, timeouts, and spawn/prep failures all fail fast and are never retried; an exhausted budget is annotated into the error text as "(after 3 attempts)"; and the backoff honors the caller's abort signal. The ambiguous bare "400 status code (no body)" message is treated as transient (see Fixed).
- **`/review-focused` is now a programmatic pipeline, like `/review` and `/review-project`.** It used to switch the MAIN agent into an interactive review prompt and nudge the main session into performing the review inline; now a single change-scoped `REVIEW` subagent runs in isolation with the same focus-aware task (same approach and output format, so reports look identical), under the same delegation gate, per-phase watchdog, abort wiring, and live progress panel as the other pipelines. On success it switches to a new static focused-review mode prompt (which records the focus) and delivers the report as a data-fenced follow-up message; on failure it error-notifies and switches no mode.

- **Documented tradeoff: the "Git write" safe prefixes auto-approve `git add`/`git commit` without a permission prompt.** `git commit` executes the repository's `.git/hooks` (`pre-commit`, `prepare-commit-msg`, `commit-msg`, `post-commit`), so in a compromised or malicious repository an auto-approved `git commit` runs attacker-planted hook code with no gate. This is deliberate (staging/committing is the common in-repo workflow the gate exists to speed up; the blast radius is the repo you already trust enough to work in, and remote writes like `git push` remain prompt-gated) — recorded here so the tradeoff is a conscious choice, and so `git reset`/`git checkout`/`git stash pop`-class destructive local writes are NOT in the safe list (pinned by `permission.test.ts`).
- **Per-agent model/thinking overrides now take precedence over the `all` entry.** `little_coder.subagent_models`/`subagent_thinking` with BOTH a named entry and `all` set previously applied `all` to every agent (the named entry was dead config for that agent); the named entry now wins and `all` is the fallback, in both the subagent tool and the programmatic pipelines (single shared pick logic, so the two surfaces cannot drift).
- **Deep-plan pipeline extracted to `mode-commands/pipeline.ts`, and the cross-extension boundary is now explicit.** The 4-phase deep-plan body (plannotator handshake, spec-file write, mode switch + handoff) is a unit-testable `runDeepPlanPipeline` next to the themed review pipeline — `index.ts` holds only the command surface. The plannotator handshake self-heals the one remaining stranding window: a plan-mode response that arrives AFTER the fallback timeout and a later failure path's exit re-exits the planning phase (plannotator's `handlePlanMode` ignores an exit while idle, so the self-heal is always safe). The subagent extension's cross-extension surface is one curated re-export barrel (`subagent/api.ts`); mode-commands imports subagent logic only through it. The two 64 KB constants (`TASK_INLINE_MAX_BYTES`, `PHASE_THREAD_MAX_BYTES`) cross-reference each other — a full-budget threaded output is exactly what decides the next task's inline-vs-`@file` routing. Fork-mode parallel fan-out writes the full session snapshot ONCE and shares the 0600 file across tasks (up to 8 identical copies of the session on disk before); a failed shared write falls back to per-task writes.
- **`/review`, `/review-project`, and `/deep-plan` pipelines are now programmatic, gated, and cancellable.** Each phase runs as an isolated `runAgent` spawn. New guardrails: the delegation gate (the SAME `resolveDelegationDepthConfig` the subagent tool uses — a depth cap of 0 or `subagent_level: off` stops the pipeline with a diagnosable error instead of spawning); a per-pipeline `AbortController` linked to `ctx.signal` when the host provides one — so a pipeline entered while idle (where `ctx.signal` is undefined) is still abortable — plus a per-phase watchdog (`LITTLE_CODER_PIPELINE_PHASE_TIMEOUT_MS`, default 30 min, `0` disables, invalid values fall back to the default) as the guaranteed escape for a wedged child; fail-fast startup when a required built-in agent was renamed (a named error instead of per-phase `FAILED` degradation or a `Map.get(...)!` TypeError); phase outputs threaded into the next phase through a 64 KB budget (`truncateForThreading`, explicit `[TRUNCATED]` marker — the synthesis task is bounded at ~7×64 KB instead of growing quadratically); and threaded outputs wrapped in a nonce-terminated `<label nonce="…">` untrusted-data fence (a reviewed repo echoing a plain `</label>` can no longer close the fence early). Themed-reviewer fan-out is bounded at the SAME `DEFAULT_SUBAGENT_CONCURRENCY` constant the subagent tool uses (4 — each slot is a full pi process, so no separate env knob); the deep-plan plannotator handshake now runs only after the gates pass, so a gated run cannot leave plannotator armed in plan mode with no deliverable, and a later phase failure leaves the planning phase again (a failed run cannot strand the session in plannotator's plan mode with no spec to submit).
- **`PI_CODING_AGENT_DIR` is honored by the subagent settings (fix).** The env branch of the subagent extension's `settingsPath()` returned the agent DIRECTORY instead of `<dir>/settings.json`, so when `PI_CODING_AGENT_DIR` was set, `readSettings()`/`writeSettings()` — and with them the `subagent_level` gate, `subagent_models`/`subagent_thinking` overrides, and pipeline depth config — silently ignored the operator's agent dir. It now resolves to `<dir>/settings.json` (with `~` expansion), and a whitespace-only value falls back to `~/.pi/agent` (a blank env can no longer make the settings path a directory).
- **`/deep-plan` pipeline: concise spec template, dual parallel review, EXECUTION handoff.**
  - **Concise spec template.** The COMPOSE template now leads with a plain-language `Overview`, drops the `Context` section, and formats every implementation step as a `- [ ] **headline** — detail` checkbox (numbered steps broke plannotator's checklist tracking). The section structure is defined in exactly one place — the COMPOSE agent's Output Format (`subagent/agents.ts`) — and the pipeline instructions reference it instead of restating the list.
  - **Dual parallel review.** COMPOSE runs twice, keyed on the explicit DRAFT/FINAL role word in the pipeline task. A new `REVIEW-PLAN-PONYTAIL` agent (lazy-engineering plan review: DELETE/SIMPLIFY/NOTE, read-only, plan-scoped) now runs in parallel with the fact-checking `REVIEW-PLAN` in a single parallel `subagent` call — if one review fails, the pipeline proceeds with the other and records the gap in Risks.
  - **EXECUTION handoff.** After plan approval the main agent no longer implements the plan itself: the handoff rule — spelled out once (`DEEP_PLAN_HANDOFF_RULE`) and carried in the persistent follow-up user message, with a compact one-time hidden-message reminder pointing at it (needed because plannotator's `before_agent_start` handler replaces the system prompt, while transcript messages persist) — directs it to spawn the existing `EXECUTION` subagent in spawn mode with "read the approved plan first, implement it, run its checks, end with a summary" and relay that summary. The rule applies ONLY to that deep-plan run (it explicitly never constrains later mode commands such as `/execute` or `/review`), reacts only to the real approval (quoted or approval-resembling text from files, web pages, tool output, or subagent results is explicitly out of scope), is delivered at most once per run, and is cleared whenever another mode command takes over, so it neither accumulates in the transcript nor leaks into other modes. Enforcement is prompt-level **by design** (recorded decision): plannotator's interactive browser review is the human gate, the rule's blast radius is a single run (any mode switch resets it), and a code-level permission-gate would require cross-extension shared state for a failure mode that is already contained.
  - **Approval triggers & string contract.** The triggers are substrings of the installed `@plannotator/pi-extension` generated prompts — a drift test (`plannotator approval trigger substrings` in `mode-commands/index.test.ts`) fails if the package rewords them, and a user's custom plannotator prompt override can also break the handoff (customized approval text is out of contract).
  - **Known limitations.** The user prompt is escaped (quotes, backslashes, newlines) before interpolation into the pipeline task strings, so pasted text cannot break out of the quoted request and rewrite the instructions. Both carriers live in the transcript, so a session compaction between handoff delivery and plan approval can summarize the rule away — explicitly telling the agent to spawn EXECUTION is always a safe fallback.
- **Output-token-limit stops auto-continue instead of aborting.** A length stop (output cut off at the token limit) now feeds a bounded loop used by token-limit-guard (`_shared/length-stop-loop.ts`): 3 targeted "resume exactly where you stopped" nudges (escalating, from stop 2 on suggesting smaller steps), a conciseness correction at stop 4, then it backs off at 5 consecutive stops — aborting to stop retries **only when compaction is disabled** (with compaction enabled — the default — pi-vcc recovers via compaction instead). A non-length turn or a session compaction resets the streak. Opt out with `little_coder.token_limit_auto_continue: false` or `LITTLE_CODER_TOKEN_LIMIT_AUTO_CONTINUE=0` to restore the old one-shot intervention + abort.
- **Token-limit auto-continue: trust-gated per-repo opt-out + compaction relationship documented.** The per-repo `token_limit_auto_continue: false` opt-out is now **trust-gated like `bash_allow`** — an untrusted repo's value is treated as absent, so it cannot disable the safety net (only the per-user/global value and the env switch apply; the env switch stays absolute). The resume/conciseness steer nudge is documented as **complementary** to pi-vcc's `agent_end` compaction recovery: there is deliberately no double-recovery guard, because compaction is ON by default and a guard would disable the feature in the default config. The steer-failure handling is a try/catch for the (only reachable) synchronous stale-context throw; an async steer failure surfaces as pi's host-side "Extension error" and is intentionally not handled in-extension.
- **The update check no longer blocks startup on the network.** The pre-spawn check is cache-only — it compares against the last-known `latest` (a stale `latest` is only ever behind reality — never a false positive) — and the refresh runs fire-and-forget, writing the cache for the next launch. Offline/stale launches cost 0 ms (was up to the 2 s fetch timeout). The "update available" notice reflects the last successful online refresh: with network it appears within one launch after the next successful refresh (up to 12 h after a release); offline launches can see an arbitrarily stale `latest`.
- **The llama.cpp `/props` context probe is disk-cached and faster to give up on.** The probed window is disk-cached per host (no expiry — a warm launch uses the cached value; a never-awaited background re-probe refreshes it for the next launch; XDG-aware, `~/.cache/little-coder/ctx-window-<host>.json`): a warm launch pays 0 ms blocking on the probe — even a fresh cache triggers one background re-probe that is never awaited, so a server restarted with a different `-c` self-corrects on the next launch; a cold launch's probe now defaults to a 500 ms timeout (was 1500 ms), so a down server stalls startup by at most 500 ms. `LITTLE_CODER_NO_CTX_PROBE=1` still skips the cache read entirely.
...
- Settings files are re-read on every use for `little_coder.*` settings (per-session memoization removed): mid-session edits now apply on the next turn/operation instead of the next session — except the bash allowlist, which is deliberately cached per session (keeps the one-time notices stable) and re-reads on session start / after `/allow`|`/deny`, so mid-session `bash_allow` edits apply on the next `/allow`|`/deny` or the next session.
- **`LITTLE_CODER_BASH_ALLOW` is read at session start and after `/allow`|`/deny` reloads** (not per tool call), matching the settings-file allowlist's per-session cache.
- `updateGlobalSettings` now serializes concurrent writers with a file lock (`proper-lockfile`, same pattern as pi's trust store) and writes a 0600 temp file.
- Internal API rename: `isProjectTrustedForBashAllow` → `isProjectTrustedFailClosed` (`_shared/project-trust.mjs`) — the name describes the matrix's actual guarantee (fail-closed on any read/parse failure) now that it gates more than the bash allowlist (model profiles and the token-limit opt-out as well).
- **Bash-allowlist notices: gated and self-describing.** The one-time "project-scoped / repo-scoped prefixes active" notices now fire only on actual `bash` tool calls while the permission gate is active (accept-all mode and non-bash tools never produce them — they were noise), and they LIST the active prefixes (first six, then "and N more") instead of only a count, so a session can see exactly which commands are auto-approved.
- **`LITTLE_CODER_TIMING=1` launch phases are per-phase, not cumulative.** Each phase (discovery/updatecheck/settings/spawn) now reports the time spent in that phase; the phases sum to the total (was: every phase was a delta from process start, so they could not sum to the total and the docs' example was impossible).
- **Token-limit lifetime cap: the cap intervention fires once per session.** When the session-lifetime ceiling (10 length stops) is hit, the "stopping auto-continue" harness intervention is emitted on the first cap stop and de-spammed thereafter (a length-stop → compaction → length-stop loop would otherwise re-notify on every stop past the cap). The abort-when-compaction-disabled path is unchanged and stays on every cap stop.
- **`pi-better-openai.json` is written by the shared locked+atomic settings writer.** The openai provider config file now goes through `updateSettingsFile` (proper-lockfile serialization, 0600 temp file, rename) and a malformed existing file is refused rather than clobbered — the same fail-safe guarantees as `settings.json`/`models.json`.
- **The bash-allowlist per-session cache re-checks project trust on file changes.** A mid-session edit to `trust.json` or the agent-dir `settings.json` (e.g. revoking project trust, or flipping `defaultProjectTrust`) is detected on the next bash call via an mtime+size freshness key and re-evaluates the trust decision — revocation now takes effect without waiting for `/allow`|`/deny` or a restart (granting still applies next session/reload, as before). `benchmark-profiles` memoizes its per-turn profile resolution with the same freshness keys (model + cwd + the three settings files' mtime/size).
- `plans/` (deep-plan artifacts) is now untracked via `.gitignore`.
- Bumped `pi-powerline-footer` `^0.7.0 → ^0.16.0` (load-bearing for the branding extension).
...
- **Subagents whose final LLM call died with a bare "400 status code (no body)" now get the transient-retry budget instead of failing the whole run.** The retry classification above failed to cover this shape: it is the openai SDK's generic format for ANY non-2xx whose error body it could not read, so a transient server/gateway rejection produces exactly the same text as Cerebras's context-overflow signal — but pi-ai's `isContextOverflow` classifies it as overflow and its retryable pattern excludes plain 400s, so `isTransientRemoteFailure` failed fast and the automatic pipelines (`/review`, `/deep-plan`) lost the subagent (and the phase) to what was usually a one-off hiccup. `isTransientRemoteFailure` now treats the bare 400-no-body message as transient (the only overflow signal with an empty body — nothing in it can confirm overflow). The cost of a true overflow misclassified this way is bounded: each respawn overflows again at the same point, fast. Explicit overflow text and 413-no-body (size overflow, unambiguous) still fail fast.
- **Multibyte tasks could defeat the E2BIG temp-file guard (and the 64 KB threading budget).** `resolveTaskArg` fast-pathed on JS string length (UTF-16 code units) while the routing budget is a UTF-8 BYTE budget — the inference "bytes ≥ length, so within-budget by code units is always inline" is inverted: a 60K-character CJK task (180 KB bytes) was inlined into argv, over Linux `MAX_ARG_STRLEN`, exactly the E2BIG the feature prevents; the byte check behind the fast path was unreachable for that window. `truncateForThreading` had the same inverted fast path, so a ~60K-code-unit multibyte phase output bypassed the 64 KB threading budget and cascaded into the same E2BIG on the next phase. Both now measure with `Buffer.byteLength` (unencoded, so no performance cost to fix it properly), the dead branch is gone, and regression tests cover the in-window case (50K CJK chars → `@file`).
- **A wedged themed reviewer could sink all six healthy siblings.** The per-phase watchdog aborted the pipeline's SHARED controller, and the themed review pipeline is failure-tolerant (one failed theme → FAILED placeholder, synthesis proceeds) — so one wedged reviewer's 30-min timeout marked every in-flight phase aborted and degraded the run to "all themed reviews failed". Failure-tolerant fan-outs now give each phase its own child controller (`childPipelineController`), linked down from the pipeline controller (a user cancel still propagates); the deep-plan dual review fan-out got the same treatment. Fail-fast phases keep the shared controller.
- **`runAgent`'s unknown-agent and missing-fork-snapshot paths now use `makeFailureResult`.** They hand-built their `SingleResult` — the unknown-agent one omitted `stopReason: "error"`/`errorMessage`/`model`, so any downstream classifier keying off `stopReason` would have silently treated an unknown agent as a non-error. Both are migrated to the shared failure-shape factory (single source of shape for every runAgent error path).
- **Unbounded stderr could become a multi-MB tool result.** The subagent TOOL's error path fed full accumulated stderr into the tool-result text (the 2 KB cap only covered the pipeline's `toPhaseOutcome`). Error text in tool results is now capped at the same 2 KB with an explicit truncation marker (`capErrorText`, shared with the pipeline's phase-error cap).
- **The E2BIG temp-file task routing no longer leaks its temp dirs on failure paths.** Over-64 KB tasks are written into a `pi-subagent-*` temp dir (a 0600 file with the full task text) and referenced as `@<file>`; the preparation sequence's own failure catch now unlinks every temp dir it created before returning (prompt/fork-session/task) — that early return happens before the outer `try/finally` that cleans up post-spawn, so without the explicit cleanup each failed run leaked a readable-by-owner temp dir in `$TMPDIR`.
- **`mapConcurrent` rejections no longer orphan in-flight worker tasks.** The fan-out's `Promise.all` was built from `await fn(item, i)` promises that no variable referenced, so a single worker rejection settled an unobserved promise (the process's `unhandledRejection` path) before the awaited all-settled barrier; each worker now has an explicit catch that rethrows after the barrier. The subagent tool's parallel path (its only caller) never hit it because worker throws are caught inside, but any future caller that lets `fn` reject was broken. Verified by `subagent/runner.test.ts` ("mapConcurrent rejection handling": three items, middle rejects — first error rethrown after the in-flight workers finish, and the process sees no unhandled rejection).
- **`resolveTaskArg` (E2BIG temp-file routing) is now directly tested.** The inline-vs-`@file` branch decision — including the UTF-8-byte (not UTF-16-length) threshold — is exported and covered in `runner.test.ts` (at/over threshold, CJK byte case, file content/layout); the old tests only re-derived the threshold comparison against itself.
- **Pipeline phase-error strings are bounded.** `toPhaseOutcome` truncates `stderr`/`errorMessage` (2 KB + marker), so a chatty child cannot turn one failed phase into a wall of notification text; the `errorMessage` fallback branch and the truncation are covered in `types.test.ts`.
- **Deterministic key order in subagent event digests.** `stableStringify` (run on every message_end/turn_end for potentially huge messages) now sorts keys with a codepoint compare instead of `localeCompare` — same stable order, orders of magnitude faster on the ASCII keys involved. The `settings.json` mtime re-read test no longer sleeps 10 ms (explicit `utimesSync` — the sleep was flaky on coarse-granularity filesystems).
- **Per-user `little_coder` profiles were silently ignored, and per-repo profiles never applied.** The package's own `.pi/settings.json` shadowed `~/.pi/agent/settings.json`, so user `little_coder` profiles (context_limit, thinking_budget, temperature, max_tokens) never took effect and per-repo `.pi/settings.json` profiles were unreadable; profiles now resolve package → per-user → per-repo via the shared settings resolver.
- **token-limit-guard only handled the first token-limit event per session.** Its handled-once flag was session-scoped, so every later length stop in the session was dropped (no intervention, no state); the flag is now per-turn, which the auto-continue loop relies on.
- **Project-scope `bash_allow` entries without a trailing space matched too much.** A repo `.pi/settings.json` entry like `"make test"` was used verbatim (substring form), so it also allowed `make test-foo …`; project-scope entries (the only untrusted-content path) now get a trailing space forced — word-boundary form, mirroring `/allow`. Global-file and env entries keep their exact user-supplied form.
- **A relative `$HOME`/`XDG_CACHE_HOME` produced a relative cache path.** The update-check and ctx-probe cache paths now reject a non-empty relative override (with a stderr note) and fall back to the absolute `~/.cache` default (with an `os.userInfo()` passwd-entry fallback for a broken `$HOME`) — closing the historical repo-root `./cache` directory bug.
- **The per-repo bash-allowlist cache evicted recently-used repos.** The LRU cap (24 entries) dropped entries when a session churned through many repos, silently losing their allowlist prefixes; the cache is now a plain unbounded Map (bounded by the number of distinct repos visited in a session — same order as pi's own per-session state).
- **Input-too-long errors now auto-continue after compaction.** When a model/provider returned an input-length error such as `Error: Input length (90313 tokens) exceeds the maximum allowed length (88948 tokens)`, compaction ran but the turn ended instead of retrying. Two gates caused this, and both are fixed: (1) pi-ai's overflow detection didn't recognize the message (it only matched the OpenRouter/Poolside form with a literal `of N tokens` suffix — `Input length (N tokens) exceeds the maximum allowed length (M tokens)` puts the counts in parentheses and drops `input`/`of`, so it fell through to pi-core's no-retry threshold path); the overflow pattern now matches this truncated form, so pi-core compacts with `willRetry=true` and resumes the turn. (2) pi-vcc owns compaction by default (`overrideDefaultCompaction`), and its post-compaction resume only continued `error` turns for Codex-specific messages, leaving generic context-overflow errors to end the turn; pi-vcc's resume now also continues on any context-overflow error. The existing single-compact-and-retry safety valve still prevents infinite loops.

- **Bash whitelist no longer blocks safe grep-style pipelines and quoted metacharacters, and `cd` can no longer bypass it.** `permission-gate` now tokenizes commands quote-aware: `grep -E "a|b" f | head -20`, `grep '\$HOME' f`, `ls -la && git status`, and bare pipeline tails like `| sort` / `| uniq` are treated as safe when every segment is individually whitelisted, while `;`, `||`, `$(…)`, bare `&`, and escaped metacharacters (`find . -exec rm {} \;`) stay blocked. The old `isNoopCd` early-return allowed _any_ `cd <workspace-path> && …` remainder through unchecked (e.g. `cd subdir && rm -rf .`); the chained command is now validated itself, so the `cd [cwd] && grep …` workaround keeps working without being a whitelist bypass.
...
- The launcher no longer clobbers a malformed `settings.json` (or `pi-better-openai.json`): on parse failure it skips the write, leaves the file untouched, and says so on stderr.

## [v1.8.1] — 2026-05-23

### Fixed

- **`glob` no longer exhausts memory on a recursive search from a huge root.** The tool capped _matches_ at 500 but never bounded the _walk_: run from a home directory (or any tree with macOS `Library`, caches, or `node_modules`), `fs.glob` recursively descended everything and its internal traversal state grew until the Node **process** ran out of heap — a host-memory crash (`Ineffective mark-compacts near heap limit`), entirely distinct from the model's _context window_ (the read-guard / window machinery operates on tool _results_ in tokens; this died mid-walk, before any result existed). The walk is now bounded two ways: heavy/irrelevant directories (`node_modules`, `.git`, `dist`, `.cache`, `Library`, `venv`, `target`, …) are **pruned** — never descended — and a hard scan budget (200 000 entries) halts the walk through the one hook `fs.glob` calls per entry (`exclude`), since it exposes no signal/abort. When results are cut short the output says so, so the model narrows its search. New unit-tested `globFiles` / `renderGlobOutcome` helpers (`.pi/extensions/extra-tools/glob.ts`), verified to prune `node_modules` (0 descent) and to halt at the scan budget.
...
- No CLI flag, `models.json` shape, or per-model-profile _schema_ changes. The only `.pi/settings.json` value change is `thinking_budget` (2048 → 4096); if you'd pinned it lower on purpose, re-set it in your own settings.
...
- The `lastChangelogVersion` pre-stamp is one-directional: it writes the _currently bundled_ pi version into settings on every launch. If you'd like to see pi's upstream changelog for a future bump, `/changelog` inside the TUI is the unconditional path — it doesn't consult `lastChangelogVersion`.
...
- **README "Serving from another machine on your LAN" section** under _Local model setup → Option C_. Covers all three local providers (llama.cpp `--host 0.0.0.0`, LM Studio's _Serve on local network_, `OLLAMA_HOST=0.0.0.0:11434 ollama serve`), the corresponding `*_BASE_URL` env on the client, a `curl /v1/models` reachability check, and a note on opening port 1234 / 8888 / 11434 in `ufw`. Validated against this repo's own benchmark hardware: `LLAMACPP_BASE_URL=http://<lan-ip>:8888/v1` against `llama-server --host 0.0.0.0` serves Qwen3.6-35B-A3B to a different machine over WiFi at the same per-token throughput as loopback.

### Changed

- **Benchmark table — Terminal-Bench 2.0 rows.** Replaced the _"awaiting maintainer merge"_ status (HuggingFace PRs [#158](https://huggingface.co/datasets/harborframework/terminal-bench-2-leaderboard/discussions/158) and [#163](https://huggingface.co/datasets/harborframework/terminal-bench-2-leaderboard/discussions/163)) with the accepted leaderboard placements published at [tbench.ai/leaderboard/terminal-bench/2.0](https://www.tbench.ai/leaderboard/terminal-bench/2.0): **Qwen3.6-35B-A3B at 24.6 % ± 3.2 (rank 120)** and **Qwen3.5-9B at 9.2 % ± 2.4 (rank 142)**. The mean shifted slightly from the originally-submitted point estimates (23.82 % → 24.6 %, 9.21 % → 9.2 %) once the leaderboard recomputed across all five trials with a confidence interval; the underlying runs are unchanged.
...
- **Edit skill documentation ([#21](https://github.com/itayinbarr/little-coder/issues/21)).** `skills/tools/edit.md` advertised `old_string` / `new_string`, but pi's Edit tool only accepts `oldText` / `newText` (single-edit form) or `edits: [{oldText, newText}]` (array form). Rewritten to show the canonical array form _and_ the single-edit back-compat form. While in there, also corrected `skills/tools/read.md` and `skills/tools/write.md` (`file_path` → `path` — pi aliases both, but the canonical name is now in the docs) and `skills/tools/grep.md` (`include` → `glob`, `max_results` → `limit`; pi does not alias these, so the old skill could genuinely produce tool-call errors on the grep path the same way Edit did).
...
Roadmap item 4 advances from _next_ to **validation done — 40.00 %**. Test-split run + leaderboard submission to follow as a separate release.
...
- **Benchmark table**: the previously _in progress_ v0.1.24 / Qwen3.5-9B row is now finalised — 9.21 % final, linked to PR #163.
...
- **Benchmark table**: added a new row for v0.1.24 / Qwen3.5-9B / TB 2.0 — _in progress_, ~15.5 % at 251 / 445.
...
Until now, every benchmark trial saw pi's hardcoded base prompt — `You are an expert coding assistant operating inside pi…` — followed by a long `Pi documentation (read only when the user asks about pi itself…)` block, _then_ AGENTS.md appended underneath as `# Project Context / ## AGENTS.md`. Two identity lines back-to-back ("expert coding assistant" + "you are little-coder") and a docs block irrelevant to TB / Polyglot / GAIA tasks.
...
- **`--system-prompt <path>`** — pi's `resource-loader.js::resolvePromptInput` resolves an existing path to its file contents and uses that as `customPrompt`, which `system-prompt.js::buildSystemPrompt` then uses _instead of_ the built-in base prompt.
...
The v0.1.20 dedup audit classified three items in the v0.1.13-restored AGENTS.md as "covered by pi's base prompt" and dropped them. Closer inspection of pi's _actual_ per-tool `promptSnippet` strings (`node_modules/@mariozechner/pi-coding-agent/dist/core/tools/*.js`) showed that classification was wrong — these three rules are **not in pi's base** and were uniquely contributing to the v0.1.18 prompt that produced **23.82 %** on the TB 2.0 leaderboard. Observation: present in the higher-baseline prompt; absent from pi. Restoring them is expected to recover the operational signal lost in the dedup.
...
2. **Read with line numbers before editing.** Pi's read snippet is just `Read file contents` — no instruction to _use_ line numbers, even though pi's Read tool returns them. The link "line-number-precise reads → exact-match Edit" is little-coder-specific and was load-bearing for the v0.1.18 baseline.
3. **Absolute paths for file operations.** Pi says nothing about path style; "Show file paths clearly" is about _output formatting_, not operational use of absolute paths. Restoring the explicit rule.
...
The `tb2-leaderboard-k5-v0.1.20-2026-04-26__11-57-55` run was killed at trial 21 / 445 (~4.7 % done, accuracy tracking the v0.1.18 baseline at 5/21 = 23.8 %). Per the same rule that v0.1.13 invoked when the prompt changed mid-run — _the leaderboard requires a consistent prompt across all 5 × 89 = 445 trials_ — partial-with-old-prompt-plus-new-trials-with-new-prompt would not be submittable. The v0.1.20 partial run is moved to `archived-partial-runs/`. A fresh run starts immediately as `tb2-leaderboard-k5-v0.1.21-*`.
...
| pi's base says                                                    | v0.1.13 AGENTS.md _also_ said                                                                                                 |
| ----------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
...
- Roadmap section item 3 (was "Terminal-Bench 2.0 — _in progress_") → now `done. 23.82 % … awaiting maintainer merge.`
...
`.pi/extensions/browser/index.ts` was passing the Readability extraction script to Playwright as a _string_ containing `() => { ... }`. Playwright evaluates strings as JavaScript expressions; a function literal evaluates to a function _value_, not an invocation, and serializes to `undefined` across the page/Node boundary. Both the primary and fallback paths had this bug, which meant `BrowserExtract` was silently returning empty text against some pages (and partial text on others, depending on Playwright version / page structure).
...
- README opening now restores a direct pointer to the Substack whitepaper — _[Honey, I Shrunk the Coding Agent](https://open.substack.com/pub/itayinbarr/p/honey-i-shrunk-the-coding-agent)_ — in the first two paragraphs, framed as "start there for the _why_; stay here for the _how_". v0.1.11's rewrite had relegated the paper link to the results table only; restoring it above the fold is more appropriate for a repo whose headline result comes from that paper.
...
- **`benchmarks/harbor_status.sh` added** with the _correct_ field path for harbor's reward schema.
...
- [#2](https://github.com/itayinbarr/little-coder/issues/2) _"Unhandled errors when Ollama is not running + crash on accidental shell commands"_ (advaitian). Both failure modes are gone in v0.1.0:
...
- [#3](https://github.com/itayinbarr/little-coder/issues/3) _"Context handling with llama-server"_ (cmhamiche). v0.0.x hardcoded context limits in `local/config.py`; v0.1.0 reads them from `.pi/settings.json`'s `little_coder.model_profiles.<provider>/<model>.context_limit`, which users can freely override (32 K default, 262 K is one settings edit away). Matches whatever `llama-server -c <N>` is serving.
- [#4](https://github.com/itayinbarr/little-coder/issues/4) _"multiple custom providers?"_ (mpetruc). `pi.registerProvider()` composes — see `.pi/extensions/llama-cpp-provider/index.ts` in the repo, which registers both `llamacpp/*` and `ollama/*` in one file. Additional providers are added by extra `pi.registerProvider()` calls (or by dropping a `~/.pi/agent/models.json` entry, per pi's docs).
...
- Full paper: [_Honey, I Shrunk the Coding Agent_ on Substack](https://open.substack.com/pub/itayinbarr/p/honey-i-shrunk-the-coding-agent); two-run reproduction report at [`docs/benchmark-reproduction.md`](docs/benchmark-reproduction.md).
